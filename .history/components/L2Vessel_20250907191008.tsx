...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, letter, skip = 0, count = 0 }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
            {items.map((seat) => (
                <TouchableOpacity key={seat} style={{ width: 32, height: 32, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: touristSeat.includes(seat) ? '#E6E2C6' : 'transparent', alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                    <Text style={{ fontSize: 10, fontWeight: 'bold' }}>{`${letter}${seat}`} </Text>
                </TouchableOpacity>
            ))} 
        </View>
    )
...
export default function L2Vessel() {
    return (
        <View style={{ width: 325, height: height + 270, backgroundColor: '#fff', marginTop: 20, paddingTop: 10, borderRadius: 50 }}>
            ...
            <View style={{ marginTop: 10, flexDirection: 'row', gap: 10, alignSelf: 'center' }}>
                <View style={{ width: '40%' }}>
            ...
            <View style={{ marginTop: 10, flexDirection: 'row', gap: 5, justifyContent: 'flex-end', alignItems: 'flex-end' }}>
                <View style={{ width: '25%' }}>
