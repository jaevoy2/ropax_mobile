...
export default function SeatPlan() {
    ...
    const seatSnapPoints = useMemo(() => {
        ...
        // Make a few snap points starting from a minimum
        return [
            Math.min(containerPercent, 100) + "%", // the container itself
            Math.min(containerPercent + 10, 100) + "%", // a little extra space
            "90%", // max
        ];
    }, [containerHeight]);
    ...
    useEffect(() => {
        if(!isFormVisible && containerHeight > 0) {
            // snap to the last snap point that fits the container
            const targetIndex = seatSnapPoints.length - 1;
