...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    const handleTripSelect = (vesselName: string, trip_id: number, origin: string, destination: string, mobileCode: string, code: string, web_code: string, departureTime: string, vesselID: number) => {
        clearTrip();
        clearPassengers();
        setFormLoading(true);
    ...
    const handleSaveCargo = () => {
        ...
        setTimeout(() => {
            ...
            if(passengers[0].cargo.cargoFare == null) {
                console.log(passengers[0].cargo.cargoFare)
    ...
    const ComputedCargoAmount = useMemo(() => {
        const cargo = passengers[0]?.cargo;
        if(!cargo) return '';

        if(cargo?.cargoType == 'Rolling Cargo') {
            if(!cargo.brandID  || !cargo.specificationID) return '';
            
            return cargoProperties.data.cargos.find(
                c => 
                    c.cargo_type_id == cargo.cargoTypeID &&
                    c.cargo_brand_id == cargo.brandID &&
                    c.cargo_specification_id == cargo.specificationID
            )?.price ?? ''
        }
        ...
        passengers[0]?.cargo?.cargoType,
        passengers[0]?.cargo?.brandID,
        passengers[0]?.cargo?.specificationID,
        passengers[0]?.cargo?.parcelCategoryID,
        cargoProperties
    ]);
            ...
            <Animated.View style={{ opacity: tripsAnimation, height: height }}> 
                {cargoContentLoading == true ? (
                    ...
                    <View style={{ paddingHorizontal: 20 }}>
                        {trips?.map((trip) => (
                            <TouchableOpacity onPress={() => handleTripSelect(trip.vessel, trip.trip_id, trip.route_origin, trip.route_destination, trip.mobile_code, trip.code, trip.web_code, trip.departure_time, trip.vessel_id)} key={trip.trip_id} style={{ paddingHorizontal: 15, paddingVertical: 20, backgroundColor: '#fff', borderRadius: 10, marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <View style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                                    ...
                                    <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                        ...
                                        <TextInput onChange={() => updateCargo(0, 'cargoFare', ComputedCargoAmount ? String(ComputedCargoAmount) : '' )} value={ComputedCargoAmount ? String(ComputedCargoAmount) : ''} editable={false} keyboardType={'numeric'} placeholder='00.00' style={{ fontWeight: 'bold', textAlign: 'right' }} />
                                    </View>
                                </View>
                            </View>
