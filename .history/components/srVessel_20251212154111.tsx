...
export default function SRVessel({ onSeatSelect }: SRVesselProps) {
    ...
    const channel = async () => {
        ...
        const listen = supabase.channel('seats_changes_channels').on('postgres_changes', { event: '*', schema: 'public', table: 'seats' }, (payload) => {
6
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 90, gap: 4, justifyContent: 'center' }}>
                    ...
                    {seatColumn1.map((seat) =>{
                        ...
                        return ( 
                            ...
                            backgroundColor: 
                                booked ? booked.station.color : isPassenger && !seatSelectionChannel.includes(seat) ? '#BA68C8' : seatSelectionChannel.includes(seat) ? '#e6d1e9ff' : seat == 'A' || seat == 'B' && !seatSelectionChannel.includes(seat) ? '#E6E2C6' : 'transparent'
                            }}> 
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 90, gap: 4, justifyContent: 'center' }}>
                    ...
                    {seatColumn3.map((seat) =>{
                        ...
                        return ( 
                            ...
                            style={{ paddingVertical: 3, width: '35%', borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, 
                            backgroundColor: 
                                booked ? booked.station.color : isPassenger && !seatSelectionChannel.includes(seat) ? '#BA68C8' : seatSelectionChannel.includes(seat) ? '#e6d1e9ff' : seat == "C" || seat == "D" ? '#E6E2C6' : 'transparent'
                            }}> 
                                <Text style={{ textAlign: 'center', fontWeight: 'bold', 
                                    color: booked || isPassenger || seatSelectionChannel.includes(seat) ? '#fff' : '#000' }}>
                                    {booked?.passTypeCode ?? seat}
