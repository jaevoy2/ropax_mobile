...
export default function Expenses() {
    ...
    const handleOnDateSelect = (selectedDate: string) => {
        ...
        setMonth(`${monthName} ${year}`);
        setDate(fulldate);
        fetchExpenses(`${monthName} ${year}`, String(date))
    }

    const fetchExpenses = async (monthName: string, fullDate?: string) => {
        const expensesfetch = await FetchExpenses(monthName, fullDate ?? '');
            ...
            <View style={{ paddingVertical: 15 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomColor: '#CF2A3A', borderBottomWidth: 1, paddingBottom: 10, paddingHorizontal: 15 }}>
                    <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                        ...
                        <Text style={{ color: '#CF2A3A', fontSize: 13, fontWeight: '900' }}>{month}</Text>
                    </View>
                    <View style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                        <Text style={{ fontSize: 10 }}>Total Month Expenses:</Text>
                        <Text style={{ color: '#CF2A3A', fontSize: 13, fontWeight: '900' }}>₱ 5000.00</Text>
                    </View>
                </View>
                ...
                <View style={{ height: height - 215 }}>
                    <ScrollView>
                        {fetchedExpenses.map((e) => (
                            <TouchableOpacity style={{ paddingHorizontal: 15, paddingVertical: 15, backgroundColor: '#f3f3f3ff', borderBottomColor: '#FFC1C5', borderBottomWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View>
                                    <Text style={{ fontWeight: '900', fontSize: 15 , color: '#cf2a3a' }}>{e.description}</Text>
                                    <Text style={{ fontWeight: 'bold', fontSize: 10, color: '#9B9B9B' }}>{e.date}</Text>
                                </View>
