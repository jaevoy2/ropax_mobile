                    ...
                    <View style={{ paddingVertical: 5 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
                                {trip.split(' ')?.[0] == 'HILONGOS' ? (
                                    <Text>{`${trip.split(' ')[0].charAt(0)}${trip.split(' ')[0].charAt(1)}${trip.split(' ')[0].charAt(2)}`}</Text>
                                ) : (
                                    <Text>{`${trip.split(' ')[0].charAt(0)}${trip.split(' ')[0].charAt(1)}${trip.split(' ')[0].charAt(3)}`}</Text>
                                )}
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
                                {trip.split(' ')?.[4] == 'HILONGOS' ? (
                                    <Text>{`${trip.split(' ')[4].charAt(0)}${trip.split(' ')[4].charAt(1)}${trip.split(' ')[4].charAt(2)}`}</Text>
                                ) : (
                                    <Text>{`${trip.split(' ')[4].charAt(0)}${trip.split(' ')[4].charAt(1)}${trip.split(' ')[4].charAt(3)}`}</Text>
                                )}
                        </View>
                    </View>
                </View>
