    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => (
                ...
                backgroundColor: bookedSeats?.find((s) => s.seat_no ==  String(`${letter}${seat}`))?.station.name === 'Hilongos Port'
                ? '#01760B'
                : bookedSeats?.find((s) => s.seat_no ==  String(`${letter}${seat}`))?.station.name === 'Hilongos Shop'
                ? '#64B5F6' :
                passengers.some((p) => p.seatNumber == String(`${letter}${seat}`)) ? '#BA68C8' : touristSeat.includes( String(`${letter}${seat}`)) ? '#E6E2C6' : 'transparent', 
                alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                    <Text style={{ fontSize: 10.5, fontWeight: 'bold',  color: bookedSeats?.find((s) => s.seat_no == `${letter}${seat}`) ? '#fff' : passengers.some((p) => p.seatNumber == Number(seat)) ? '#fff' : '#000' }}>
                    {bookedSeats?.find((s) => s.seat_no == String(`${letter}${seat}`))?.passTypeCode ?? `${letter}${seat}`}
                    </Text>
                </TouchableOpacity>
            ))} 
        </View>
