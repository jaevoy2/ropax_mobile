                ...
                {!passengers.some((p) => p.passType == 'Passes') && passengers.some((p) => p.seatNumber != null) && (
                    <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={passengers.length > 0 ? 0 : -1} bottomInset={1} onChange={(index) => { handleSheetChange(index)}}>
                        ...
                        <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: 10, paddingBottom: 10 }} showsVerticalScrollIndicator={true}>
                            ...
                            {isFormVisible == false ? (
                                <TouchableOpacity onPress={() => onFormViewButton()} disabled={passengers.length == 0} style={{ backgroundColor: passengers.length == 0 ? '#df5a68ff' : '#cf2a3a', width: '100%', alignSelf: 'center', borderRadius: 30, paddingVertical: 15, marginTop: 15 }}>
