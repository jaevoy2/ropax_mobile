                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 5, marginBottom: 5 }}>
                    {trips?.map((t) => (
                        <TouchableOpacity onPress={() => setSelectedTrip(t.trip_id)} key={t.trip_id} style={{ backgroundColor: selectedTrip == t.trip_id ? '#cf2a3a' : 'transparent', borderColor: selectedTrip == t.trip_id ? 'transparent' : '#cf2a3a', borderWidth: 1, paddingVertical: 6, paddingHorizontal: 5, borderRadius: 5  }}>
                            <View>
                                <Text style={{ fontWeight: 'bold', fontSize: 12, color: selectedTrip  == t.trip_id ? '#fff' : '#cf2a3a' }}>{`${t.departure}`}</Text>
                                <Text style={{ fontWeight: 'bold', fontSize: 12, color: selectedTrip == t.trip_id ? '#fff' : '#cf2a3a' }}>{`${t.route_origin} > ${t.route_destination} [ ${t.vessel} ]`}</Text>
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>
