...
export default function Forms({ errorForm }: FormProps) {
    ...
    useEffect(() => {
        ...
        const passes = passengers.find(p => p.passType == 'Passes')
        console.log(passes);
        passengerType();
    }, []);
            ...
            <View style={{ height: '82%', marginTop: 10 }}>
                <ScrollView>
                    {passengers.map((p, index) => (
                        <View key={index} style={{ position: 'relative', borderColor: errorForm.includes(p.seatNumber ?? '') ? '#cf2a3a' : '#B3B3B3', borderWidth: 1, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 10, marginBottom: 20, backgroundColor: '#fff' }}>
