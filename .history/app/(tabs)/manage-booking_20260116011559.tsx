...
export default function ManageBooking() {
    ...
    const fetchBooking = async (dateString: string) => {
        try {
            ...
            console.log(response.data[0].bookings.trip_schedule.trip.departure_time);
            if(!response.error) {
                const paxData: PaxBookingProps[] = response.data.map((passenger: any) => ({
                    id: passenger.id,
                    name: `${passenger.first_name} ${passenger.last_name}`,
                    departureDate: new Date(passenger.bookings.created_at).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
                    }),
                    departureTime: new Date(`1970-01-01T${passenger.bookings.trip_schedule.trip.departure_time}`).toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit',
                        hour12: true
                    }),
                    route: passenger.bookings.trip_schedule.trip.route.mobile_code,
                    vessel: passenger.bookings.trip_schedule.trip.vessel.name,
                    referenceNumber: passenger.bookings.reference_no
                }))
