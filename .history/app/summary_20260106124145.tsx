...
export default function PaymentSummary() {
    ...
    const { id, note, totalFare, fareChange, webCode, destination, origin, departure_time, vessel, setRefNumber, setFareChange, setCashTendered } = useTrip();
    ...
    const handleConfirmation = async () => {
        ...
        try {
            console.log(passengers.flatMap(p => p.cargo))
                            ...
                            {passengers.map((p, index) => (
                                <View key={`${p.accommodationID}-${index}`} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginLeft: 15 }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <Text style={{ fontSize: 12, width: '40%', fontWeight: '400' }}>{`${p.name?.split(',')[1]?.trim().charAt(0)}. ${p.name?.split(',')[0]} `}</Text>
                                        <Text style={{ fontSize: 12, width: '40%', fontWeight: '400', color: '#4b4b4bff' }}>{`(${p.passType})`}</Text>
                                    </View>
                                ...
                                p.hasInfant && p.infant?.map((i, index) => (
                                    <View key={`${p.accommodationID}-${index}`} style={{ marginBottom: 3 }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <View style={{ flexDirection: 'row' }}>
                                                <Text style={{ fontSize: 12, width: '40%', fontWeight: '400' }}>{`${i.name?.split(',')[1]?.trim().charAt(0)}. ${i.name?.split(',')[0]} `}</Text>
                                                <Text style={{ fontSize: 12, width: '40%', fontWeight: '400', color: '#4b4b4bff' }}>{`(Infant)`}</Text>
                                            </View>
                                            <Text style={{ fontSize: 12, fontWeight: 'bold' }}>₱ 00.00</Text>
                                        </View>
                                    </View>
                                ))
                            )}
                            ...
                            {passengers.length == 0 ? (
                                <>
                                    {paxCargoProperty.map((cargo: any) => (
                                        <View key={cargo.id} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginLeft: 15, }}>
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                                                ...
                                                <Text style={{ fontSize: 12, fontWeight: 'bold'}}>{cargo?.cargoAmount.toLocaleString('en-PH', { minimumFractionDigits: 2,  maximumFractionDigits: 2 })}</Text>
                                            </View>
                                        </View>
                                    ))}
                                </>
                                ...
                                <>
                                    {passengers.flatMap(p => p.hasCargo ? 
                                        p.cargo.map(c => (
                                            <View key={c.id} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginLeft: 15, }}>
                                                <View style={{ flexDirection: 'row', gap: 3 }}>
                                                    {c.quantity > 1 && (
