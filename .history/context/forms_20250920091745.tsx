...
export default function Forms({ errorForm }: FormProps) {
    ...
    const hasInfantChecker = (seat: number | string, hasInfantValue: boolean, passType_id: number) => {
        ...
        if(!currentValue) {
            ...
            addInfant(seat, {name: '', gender: '', age: 0, type_id: id})
        }
    }
                ...
                <ScrollView>
                    {passengers.map((p) => (
                        <View key={p.seatNumber} style={{ borderColor: errorForm.includes(p.seatNumber ?? '') ? '#cf2a3a' : '#B3B3B3', borderWidth: 1, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 10, marginBottom: 20, backgroundColor: '#fff' }}>
                            ...
                            {p.hasInfant && (
                                <>
                                    <TouchableOpacity onPress={() => addInfant(p.seatNumber!, {name: '', gender: '', age: 0, type_id: passengerType?.find((i) => i.name == 'Infant')?.id!})} style={{ backgroundColor: '#cf2a3a', borderColor: '#cf2a3a', borderWidth: 1, padding: 5, borderRadius: 5, alignSelf: 'flex-end', marginTop: 10, marginBottom: -15, flexDirection: 'row', gap: 5, alignItems: 'center' }}>
