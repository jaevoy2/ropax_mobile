...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    useEffect(() => {
        if (!cargoProperties) return;

        let updatedCargo = cargoOnlyProperty.map(c => {
            const amount = Number(ComputedCargoAmount(c)) || 0;
            return { ...c, cargoAmount: amount };
        });

        setCargoOnlyProperty(updatedCargo);
    }, [cargoProperties]);

    const cargoTotalAmount = useMemo(() => {
        return cargoOnlyProperty.reduce(
            (sum, c) => sum + (Number(c.cargoAmount) || 0),
            0
        );
    }, [cargoOnlyProperty]);
                            ...
                            <View style={{ flexDirection: 'column', alignItems: 'flex-start', marginTop: 10 }}>
                                ...
                                <View style={{ borderColor: '#cf2a3a', backgroundColor: '#cf2a3b1a', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 8 }}>
                                    ...
                                    <Text style={{ fontWeight: 'bold', textAlign: 'right', fontSize: 16 }}>
                                        {cargoTotalAmount.toFixed(2) ?? '0.00'}
                                    </Text>
                                </View>
                            </View>
                        )}
