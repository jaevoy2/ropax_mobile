...
export default function ManageBooking() {
    ...
    const fetchBooking = async (dateString: string) => {
        try {
            ...
            if(!response.error) {
                console.log(response.data[0].bookings.find((t: any) => t.trip_schedule).trip_schedule.trip.route.mobile_code);
                const paxData: PaxBookingProps[] = response.data.map((passenger: any) => ({
                    ...
                    departureTime: new Date(`1970-01-01T${passenger.bookings.find((t: any) => t.trip_schedule)?.trip_schedule.trip.departure_time}`).toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit',
                        hour12: true
                    }),
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <View style={{ justifyContent: 'center', paddingTop: 20, paddingHorizontal: 20, height: height - 170 }}>
                ...
                {/* <FlatList data={passengers} keyExtractor={(passengers) => passengers.referenceNumber} showsVerticalScrollIndicator={false} renderItem={({ item }) => (
                    <View key={item.id} style={{ borderColor: '#B3B3B3', borderWidth: 1, backgroundColor: '#fff', borderRadius: 8, padding: 8, marginBottom: 10 }}>
