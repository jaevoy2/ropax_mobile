...
export default function SeatPlan() {
    ...
    const [passessAccomID, setPassesAccomID] = useState(0);
    const [passessAccomName, setPassesAccomName] = useState('');
    ...
    useEffect(() => {
        ...
        const passesAccomType = async () => {
            try {
                const response = await FetchPassengerType();
    
                if(response) {
                    const passesTypeAcom = response.data.find((type: any) => type.name == 'Passes' || type.code == 'P');

                    if(passesTypeAcom) {
                        setPassesAccomID(passesTypeAcom.id);
                        setPassesAccomName(passesTypeAcom.name);
                    }else {
    ...
    const passessToggleSheet = () => {
        if(passengers.length < 1) {
            setPassengers(prev => [...prev, { accommodationID: passessAccomID, accommodation: passessAccomName }]);
        }
    ...
    const handleRemoveSeat = useCallback((seat: number | string | undefined) => {
        setPassengers((prev) => {
            const newSeats = prev.filter((s) => s.seatNumber !== seat);

            // If no passengers left, reset everything in one batch
            if (newSeats.length === 0) {
            React.startTransition(() => {
                setIsExpanded(false);
                clearPassengers();
                setPassesDisabled(false);
                setContentHeight(height);
                setSheetHeight(0);
            });

            // Start animation *after* the transition (small timeout syncs UI)
    ...
    }, [height, translateY, clearPassengers]);
    ...
    const handleSave = () => {
        ...
        setTimeout(() => {
            ...
            const passesTypeAccom = passengers.some((p) => p.accommodation == 'Passes' || p.passTypeCode == 'P');
    ...
    return (
        <View style={{ height: '100%', position: 'relative' }}>
            ...
            <TouchableOpacity disabled={passesDisabled} onPress={() => passessToggleSheet()} style={{ position: 'absolute', right: 20, top: 50, zIndex: 5 }}>
            ...
            <Animated.View style={{ position: 'absolute', bottom: 0, width: width, height, backgroundColor: '#f8f8f8ff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 10, transform: [{ translateY }], zIndex: 5 }}>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, borderRadius: 10, }}>
                    <View style={{flexDirection: 'row', flexWrap: 'wrap', gap: 8, borderColor: '#B3B3B3', borderWidth: 1, backgroundColor: '#fff', borderRadius: 8, padding: 8, width: '100%', marginTop: 5 }}>
                        {passengers.filter((passengers) => passengers.accommodation != 'Passes').map((p) => (
            ...
            <Animated.View style={{ position: 'absolute', bottom: 0, width: width, height, backgroundColor: '#f8f8f8ff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 10, transform: [{ translateY: passessFormAnim }], zIndex: 5 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <TouchableOpacity onPress={() => addPasses()} style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#cf2a3a', borderRadius: 5, padding: 10 }}>
                ...
                {isPassFormVisible == true && (
                    <View style={{ paddingVertical: 10, position: 'relative' }}>
                        ...
                        <TouchableOpacity onPress={() => handleSave()} style={{ position: 'absolute', bottom: 70, backgroundColor: '#cf2a3a', width: '100%', alignSelf: 'center', borderRadius: 30, paddingVertical: 15, zIndex: 5 }}>
                            {saveloading == true ? (
            ...
            {/* buttons */}
            {isExpanded == true && isFormVisible == false && isPassFormVisible != true  && (
