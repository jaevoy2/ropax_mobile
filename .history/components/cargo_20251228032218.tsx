...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    const { cargoProperties, cargoOnlyProperty, setCargoProperties, updateCargoOnlyProperty } = useCargo();
    ...
    const handleSaveCargo = () => {
        ...
        setTimeout(() => {
            ...
            if(cargoOnlyProperty.cargoType == 'Rolling Cargo' && !passengers[0].cargo?.brand?.trim()) {
                Alert.alert('Invalid', 'Brand is required.');
                setSaveCargoLoading(false);
                return;
            }
            if(cargoOnlyProperty.cargoType == 'Rolling Cargo' && !passengers[0].cargo?.specification?.trim()) {
                Alert.alert('Invalid', 'CC is required.');
                setSaveCargoLoading(false);
                return;
            }
            if(cargoOnlyProperty.cargoType == 'Rolling Cargo' && !passengers[0].cargo?.plateNo?.trim()) {
                Alert.alert('Invalid', 'Plate number is required.');
                setSaveCargoLoading(false);
                return;
            }

            if(cargoOnlyProperty.cargoType == 'Parcel' && !passengers[0].cargo?.parcelCategory.trim()) {
                Alert.alert('Invalid', 'Parcel category is required.');
                setSaveCargoLoading(false);
                return;
            }

            if(cargoOnlyProperty.cargoType == 'Animal' && !passengers[0].cargo?.parcelCategory.trim()) {
                Alert.alert('Invalid', 'Parcel category is required.');
                setSaveCargoLoading(false);
                return;
            }
    ...
    useEffect(() => {
        updateCargoOnlyProperty('cargoAmount', ComputedCargoAmount);
    }, [ComputedCargoAmount])
                                            ...
                                            <View style={{ marginTop: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                                ...
                                                <View style={{ width: '48%' }}>
                                                    ...
                                                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5, paddingVertical: 1 }}>
                                                        <TextInput onChangeText={(text) => updateCargoOnlyProperty('cargoPlateNo', text)} placeholder='Plate#' style={{ fontSize: 13 }} />
                                                    </View>
                                                </View>
                                            </View>
                                        </>
