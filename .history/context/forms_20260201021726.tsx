...
export default function Forms({ errorForm }: FormProps) {
    ...
    const handleGetPaxFare = (passenger: PassengerProps) => {
        ...
        console.log('pax: ', paxFares.find(p => p.passenger_type_id == passenger.passType_id))
        return prop?.fare ?? 0
    }
                                    ...
                                    {typeLoading == true ? (
                                        ...
                                        <>
                                            {passengerType?.filter((t) => t.name != 'Infant' && t.name != 'Passes')
                                                .map((type) => (
                                                <TouchableOpacity onPress={() => {updatePassenger(index, p.seatNumber, 'passType', type.name), updatePassenger(index, p.seatNumber, 'passType_id', type.id), updatePassenger(index, p.seatNumber!, 'passTypeCode', type.code)}}
