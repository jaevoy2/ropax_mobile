...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    const ComputedCargoAmount = (cargo: CargoOnlyProperties) => {
        if(!cargo || !cargoProperties) return 0;

        if(cargo?.cargoType == 'Rolling Cargo') {
            if(!cargo.cargoBrandID  || !cargo.cargoSpecificationID) return 0;
        ...
        if (cargo.cargoType === 'Parcel') {
            if (!cargo.parcelCategoryID) return 0;
            return Number(
                cargoProperties.data.cargo_options?.find(
                    c => c.parcel_category_id === cargo.parcelCategoryID &&
                        c.route_id === routeID
                )?.price ?? 0
            );
        }
        ...
        if (cargo.cargoType === 'Animal/Pet') {
            const animalTypeId = cargoProperties.data.cargo_types?.find(t => t.name === 'Animal/Pet')?.id;
            return Number(
                cargoProperties.data.cargo_options?.find(
                    c => c.cargo_type_id === animalTypeId &&
                        c.route_id === routeID
                )?.price ?? 0
            );
        }
...
useEffect(() => {
    const total = cargoOnlyProperty.reduce((sum, c) => sum + ComputedCargoAmount(c), 0);
    setTotalAmount(total);
}, [cargoOnlyProperty, cargoProperties, routeID]);

    useEffect(() => {
        const total = cargoOnlyProperty.reduce((sum, c) => {
            let subtotal = sum + Number(c.cargoAmount || 0.00);

            console.log(c.cargoAmount);
            return subtotal;
        }, 0);
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                                        ...
                                        <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 8 }}>
                                            <Text style={{ fontSize: 16 }}>₱ </Text>
                                            <Text style={{ fontWeight: 'bold', textAlign: 'right', fontSize: 16 }}>
                                                {ComputedCargoAmount(c).toFixed(2)}
                                            </Text>
                                        </View>
                                    </View>
                                    {c.id > 1 && (
                                        ...
                                        { c?.cargoType == 'Rolling Cargo' ? (
                                            <>
                                                ...
                                                <View style={{ marginTop: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                                    <View style={{ width: '48%' }}>
                                                        <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#545454' }}>{c.specificationID}:</Text>
                                                        <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
