...
export default function SeatPlan() {
    ...
    const handleSeatSelect = () => {
        if(passengers.length == 0 || (passengers.length > 0 && passengers[0].seatNumber != null)) {
    ...
    const handleRemoveSeat = (seat: number | string | undefined) => {
        ...
        if(hasCargo && paxCargoSeat?.seatNumber == seat) {
            setPassengers(prev => 
                prev.map(p => p.hasCargo ? { ...p, seatNumber: '' }: p)
            );
        }

        if(newSeats.length == 0 || paxCargoSeat?.seatNumber == null) {
        ...
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={{ height: '100%', overflow: 'hidden' }}>
                ...
                {!passengers.some((p) => p.passType == 'Passes') && passengers.some((p) => p.seatNumber != null) && (
                    <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={passengers.length > 0 && !passengers.some(p => p.seatNumber == null) ? 0 : -1} bottomInset={1} onChange={(index) => handleSheetChange(index)}>
