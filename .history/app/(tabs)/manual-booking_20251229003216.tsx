...
import { ActivityIndicator, Alert, Animated, AppState, Dimensions, KeyboardAvoidingView, Modal, Platform, RefreshControl, ScrollView, Text, TouchableOpacity, View, } from "react-native";
...
export default function ManualBooking() {
    const { id, vessel, routeID, setRouteID, setVessel, setID, setOrigin, setDestination, setVesselID, setCode, setWebCode, setDepartureTime, setMobileCode } = useTrip();
    ...
    const handleSaveTrip = (vesselName: string, trip_id: number, routeId: number, origin: string, destination: string, mobileCode: string, code: string, web_code: string, departureTime: string, vesselID: number) => {
        setLoading(true);          
        setTimeout(() => {
            if(trip_id != id) {
                passengers.forEach( passenger => {
    ...
    return (
        <View style={{ backgroundColor: '#f1f1f1', height: height, position: 'relative' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', borderBottomColor: '#FFC107', borderBottomWidth: 1 }}>
                {bookingTypes.map((bt) => (
                    <TouchableOpacity onPress={() => setBookingType(bt.type)} key={bt.type} style={{ flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 8, justifyContent: 'center', width: '50%', backgroundColor: bookingType == bt.type ? '#FFC107' : '#ffc1071f' }}>
            ...
            <View>
                <View style={{ flexDirection: 'row', gap: 10, paddingHorizontal: 20, paddingTop: 20 }}>
                ...
                <ScrollView refreshControl={ <RefreshControl refreshing={refresh} onRefresh={handleRefresh} colors={['#cf2a3a']} progressViewOffset={180} />} nestedScrollEnabled={true} >
                    {bookingType == 'Walk-In' ? (
                        <View style={{ paddingHorizontal: 20 }}>
                            {contentLoading == true ? (
                                ...
                                <>
                                {trips?.map((trip) => (
                                    <TouchableOpacity onPress={() => handleSaveTrip(trip.vessel, trip.trip_id, trip.route_id, trip.route_origin, trip.route_destination, trip.mobile_code, trip.code, trip.web_code, trip.departure_time, trip.vessel_id)} key={trip.trip_id} style={{ paddingHorizontal: 15, paddingVertical: 20, backgroundColor: '#fff', borderRadius: 10, marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <View>
                        ...
                        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1, position: 'relative', zIndex: 1000, top: -50 }}>
