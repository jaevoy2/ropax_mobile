...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, onSeatSelect }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
            {items.map((seat) => (
                <TouchableOpacity onPress={() => onSeatSelect?.(seat)} key={seat} style={{  paddingVertical: 2, width: 32, height: 32, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: touristSeat.includes(seat) && passengers.some((p) => p.seatNumber != Number(seat) ) ? '#E6E2C6' : passengers.some((p) => p.seatNumber == Number(seat)) ? '#BA68C8' : 'transparent', alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                    <Text style={{ fontSize: 14, fontWeight: 'bold', color: passengers.some((p) => p.seatNumber == Number(seat)) ? '#BA68C8' : '#000' }}>{seat}</Text>
                </TouchableOpacity>
            ))} 
        </View>
    )
...
export default function SRVessel({ onSeatSelect }: SRVesselProps) {
    ...
    return (
        <View style={{ width: 330, height: height + 290, backgroundColor: '#FAFAFA', marginTop: 10, paddingTop: 10, paddingBottom: 50, borderRadius: 50 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 10, width: '80%', alignSelf: 'center' }}>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 90, gap: 4, justifyContent: 'center' }}>
                    ...
                    {seatColumn1.map((seat) => ( 
                        <TouchableOpacity onPress={() => assignseat?.(seat)} key={seat} style={{ paddingVertical: 3, width: '35%', borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: seat == 'A' || seat == 'B' ? '#E6E2C6' : 'transparent' }}> 
