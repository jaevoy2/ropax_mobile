    ...
    const handleFetchTrips = async (queryDate: string) => {
        try {
            ...
            function verifyTime(timeString: string, specificDay: string) {
                ...
                const tripTime = new Date(`${specificDay}T00:00:00`);
                tripTime.setHours(hours, minutes, 0, 0);

                console.log('Current Time:', currentTime,  'Trip Time:', tripTime);
            ...
            if(tripsFetch) {
                const tripsData: TripProps[] = tripsFetch.data.map((t: any) => ({
                    ...
                    hasDeparted: (verifyTime(t.trip.departure_time, t.trip.specific_days), tripStatus == 'departed' ? true : false)
                }))
                            ...
                            {contentLoading == true ? (
                                ...
                                <>
                                    ...
                                    {trips.some(t => t.hasDeparted == true) && (
                                        <>
                                            <Text style={{ color: '#7A7A85', marginTop: 25, fontWeight: 'bold' }}>Trips Departed</Text>
