...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
        ...
        switch(passengerCargoProps?.cargo?.cargoType) {
            case 'Rolling Cargo':
                ...
                if(paxCargoProps.brand != null && paxCargoProps.specification != null) {
                    const amount = cargoProperties.data.cargos.find((cargo: any) => cargo.cargo_type_id == paxCargoProps.cargoTypeID && cargo.cargo_brand_id == paxCargoProps.brandID && cargo.cargo_specification_id == paxCargoProps.specificationID);
                    if(amount?.price != null) {
                        updateCargo(0,  'cargoFare', amount.price ?? '')
                        return;
                    }
                }
                break;
            ...
            case 'Parcel':
                ...
                if(paxCargoProps.parcelCategory != null) {
                    const amount = cargoProperties.data.cargos.find((cargo: any) => cargo.parcel_category_id == paxCargoProps.parcelCategoryID);
                    if(amount?.price != null) {
                        updateCargo(0,  'cargoFare', amount.price ?? '');
                        return;
                    }
    ...
    }, [passengers[0]?.cargo?.brand, passengers[0]?.cargo?.specification, passengers[0]?.cargo?.parcelCategory, passengers[0]?.cargo?.cargoType, passengers[0]?.cargo?.parcelCategoryID ]);
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <View style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                                    ...
                                    <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                        ...
                                        <TextInput onChangeText={(text) => updateCargo(0, 'cargoFare', text)} value={passengers[0]?.cargo?.cargoFare} keyboardType={'numeric'} placeholder='00.00' style={{ fontWeight: 'bold', textAlign: 'right' }} />
                                    </View>
                                </View>
                            </View>
