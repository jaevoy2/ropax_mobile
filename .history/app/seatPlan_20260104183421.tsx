...
export default function SeatPlan() {
    ...
    useEffect(() => {
        ...
        totalFare = paxFare;
        console.log(paxCargoProperty.map(c => c.cargoAmount == 0))
        if(passengers.some(p => p.hasCargo == true)) {
            const cargTotalAmout = paxCargoProperty.reduce((total, c) => {
                console.log('cargo fare: ');
                return total + Number(c.cargoAmount);
            }, 0)

            totalFare += cargTotalAmout
        }
