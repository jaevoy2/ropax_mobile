...
export default function Forms({ errorForm }: FormProps) {
    ...
    const { paxCargoProperty, cargoProperties, updatePaxCargoProperty } = useCargo();
    ...
    const hasCargoChecker = useCallback((seat, passIndex) => {
        ...
        setPassengers(prev => 
            prev.map((p, index) => {
                ...
                return {
                    ...p, hasCargo: isHasCargo, cargo: isHasCargo ? [{cargoAmount: 0.00}] : []
                }
            })
        )
    }, []);
    ...
    useEffect(() => {
        computedCargoList.forEach((c) => {
            const original = paxCargoProperty.find(p => p.id == c.id);
            if(!original) return;
                        ...
                        {p.hasCargo == true && (
                            <View style={{ marginTop: 25 }}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                                    <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#cf2a3a', marginBottom: 5 }}>Cargo Details</Text>
                                    ...
                                    <View key={`${p.name}-${index}`}>
                                        ...
                                        <View style={{flexDirection: 'row', justifyContent: 'space-between' }}>
                                            <View style={{ width: '70%' }}>
                                                ...
                                                <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                    <Dropdown onChange={(item) => {updateCargo( p.seatNumber ?? null, index, cargoIndex, 'cargoType', item.label), updateCargo(p.seatNumber ?? null, index, cargoIndex, 'cargoTypeID', item.value)}} value={c.cargoTypeID} data={cargoProperties?.data.cargo_types.map((type: any) => ({ label: type.name, value: type.id })) } labelField="label" valueField="value" placeholder="Select Cargo Type" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                        containerStyle={{
                                        ...
                                        {c.cargoType == 'Rolling Cargo' ? (
                                            ...
                                            <View style={{ marginTop: 5 }}>
                                                ...
                                                <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                    ...
                                                    value={c.parcelCategoryID} data={cargoProperties?.data.parcel_categories.map((category: any) => ({ label: category.name, value: category.id })) } labelField="label" valueField="value" placeholder="Select Category" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                        containerStyle={{
                                                            alignSelf: 'flex-start',
                                                            width: '85%',
                                                        }}
