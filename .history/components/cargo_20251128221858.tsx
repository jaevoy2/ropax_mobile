...
export default function CargoComponent() {
    ...
    const handleSaveTrip = (vesselName: string, trip_id: number, origin: string, destination: string, mobileCode: string, code: string, web_code: string, departureTime: string, vesselID: number) => {
        ...
        setTimeWithRoute(`${departure_time} | ${origin} --- ${destination}`)
        toggleFormSheet();
        setTimeout(() => {
                    ...
                    <View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <View style={{ flexDirection: 'column' }}>
                                ...
                                <Text style={{ color: '#cf2a3a', fontSize: 24, fontWeight: '900', marginTop: -5 }}>{selectedVessel}</Text>
                            </View>
                            <TouchableOpacity onPress={() => closeFormSheet()} style={{ flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 8, paddingHorizontal: 15, borderColor: '#cf2a3a', borderRadius: 5, borderWidth: 1 }}>
                                <Ionicons name={'swap-horizontal'} color={'#cf2a3a'} size={15} />
                                <Text style={{ color: '#cf2a3a', fontWeight: 'bold', fontSize: 13 }}>Change Route</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
