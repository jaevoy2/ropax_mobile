...
export default function L2Vessel({onSeatSelect}: L2VesselProps) {
    ...
    const channel = async () => {
        ...
        const listen = supabase.channel('seats_changes_channels').on('postgres_changes', { event: '*', schema: 'public', table: 'seats_channels' }, (payload) => {
            const newData: any = payload.new;
            const { event_type, seat_number, trip_id, device_id } = newData;

            setSeatSelectionChannel(prev =>
                event_type === "selected" && trip_id == id && deviceID != device_id
                    ? [...prev, seat_number]
