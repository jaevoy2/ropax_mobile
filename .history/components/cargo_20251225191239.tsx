...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    const { cargo_types, brands, specifications, parcel_categories, cargos } = cargoProperties.data || null;
    ...
    useEffect(() => {
        const passengerCargoProps = passengers[0];

        switch(passengerCargoProps.cargo.cargoType) {
            case 'Rolling Cargo':
                const props = passengerCargoProps.cargo;
                if(!props.brand.trim() && !props.specification) {
                    // const total = cargos.filter((cargo: any) => cargo.cargoTypeID == )
                }
        }
