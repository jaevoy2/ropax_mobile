...
export default function PaymentSummary() {
    ...
    useEffect(() => {
        if(cashTendered != 0) {
            if(!cargoOnlyProperty.cargoAmount) {
                const change = cashTendered - totalFare;
                setFareChange(change);
            }else{
                const change = cashTendered - Number(cargoOnlyProperty.cargoAmount);
                setFareChange(change);
            }
        }
    }, [cashTendered]);
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            <View style={{ width: '80%' }}>
                                ...
                                <View style={{ marginLeft: 15, flexDirection: 'row', gap: 3 }}>
                                    <Text style={{ fontSize: 12, color: '#858585ff' }}>{`${cargoOnlyProperty.cargoBrand} ${cargoOnlyProperty.cargoSpecification}` || cargoOnlyProperty.parcelCategory}</Text>
                                    <Text style={{ fontSize: 12, color: '#858585ff' }}>{`(${cargoOnlyProperty.cargoType})`}</Text>
                                </View>
                            </View>
