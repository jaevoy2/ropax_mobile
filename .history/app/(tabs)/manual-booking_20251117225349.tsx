        ...
        try {
            ...
            if(!totalBookingFetch.error) {
                const totalBookingFetchData: TotalBookingProps[] = totalBookingFetch.data.map((t: any) => ({
                    station: t.station,
                    accommodation: t.accommodations.map((a: any) => ({
                ...
                <View style={{ padding: 10 }}>
                    {totalSheetLoading == true ? (
                        ...
                        <>
                            ...
                            <View style={{ flexDirection: 'row', gap: 5, alignItems: 'center', flexWrap: 'wrap', marginTop: 10 }}>
                                {trips?.map((trip) => (
                                    <TouchableOpacity key={trip.trip_id} style={{ paddingHorizontal: 18, paddingVertical: 5, backgroundColor: bottomSheetTripID == trip.trip_id ? '#cf2a3a' : '#fff', borderRadius: 5, marginTop: 12, flexDirection: 'row', alignItems: 'center', borderColor: '#cf2a3a', borderWidth: 1 }}>
                                        <View>
                                            <Text style={{ fontWeight: 'bold', fontSize: 13, color:  bottomSheetTripID == trip.trip_id ? '#fff' : '#cf2a3a' }}>{`${trip.departure}`}</Text>
                                            <Text style={{ fontWeight: 'bold', fontSize: 12, color:  bottomSheetTripID == trip.trip_id ? '#fff' : '#cf2a3a' }}>{`${trip.mobile_code} [ ${trip.code} ]`}</Text>
                                        </View>
                                    </TouchableOpacity>
                                ))}
                            </View>
                            <View style={{ marginTop: 20 }}>
                                <Text style={{ fontWeight: 'bold', fontSize: 16, color: '#cf2a3a' }}>30 TOTAL PAYING PASSENGERS</Text>
                            </View>
                        </>
                    )}
                </View>
