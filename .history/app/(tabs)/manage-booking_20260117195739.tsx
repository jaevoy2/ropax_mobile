    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
            <View style={{ paddingTop: 50, height: 150, backgroundColor: '#cf2a3a', paddingHorizontal: 20, gap: 15 }}>
                ...
                <View style={{ borderColor: '#cfcfcf73', backgroundColor: onfilter ? '#fff' : '#d4abab73', borderWidth: 1, borderRadius: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10 }}>
                    <TextInput onFocus={() => setOnFilter(true)} onBlur={() => setOnFilter(false)} placeholder='Search' placeholderTextColor={onfilter ? '#000' : '#fff'} style={{ fontSize: 13, fontWeight: '600', width: '90%', color: '#000' }} />
                    <TouchableOpacity style={{ borderLeftWidth: 1, borderLeftColor: '#cfcfcfd0', paddingLeft: 10 }}>
                        <Ionicons name={'search'} size={20} color={onfilter ? '#000' : '#fff'} />
                    </TouchableOpacity>
                </View>
            </View>
                    ...
                    <>
                        {passengers.length > 0 ? (
                            <>
                                ...
                                <FlatList initialScrollIndex={passengers.length - 1} inverted={true} data={passengers} keyExtractor={(passengers) => String(passengers.id)} showsVerticalScrollIndicator={false} 
