...
import { CargoProperties, PaxCargoProperties, useCargo } from '@/context/cargoProps';
...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    const { vessel, routeID, totalFare, setTotalFare, setRouteID, setVessel, setID, setOrigin, setDestination, setVesselID, setCode, setWebCode, setDepartureTime, setMobileCode, clearTrip } = useTrip();
    const { totalAndNote, cargoProperties, paxCargoProperty, setTotalAndNote, updateAmountAndNote, setCargoProperties, setPaxCargoProperties, updatePaxCargoProperty } = useCargo();
    ...
    const [cargoAmountState, setCargoAmountState] = useState<CargoAmounts[]>([])
    ...
    const handleCargoQuantity = (operation: string, id: number) => {
        const cargo = paxCargoProperty.find(c => c.id == id);
        if(!cargo) return;

        if(operation == 'add') {
           cargo.quantity += 1;
           console.log(cargo.quantity += 1);
        }

        if(operation == 'minus') {
            if(cargo.quantity == 1) return;
    ...
    const getTotalAmount = useMemo(() => {
        return paxCargoProperty.reduce(
            (sum, c) => sum + Number(ComputedCargoAmount(c)),
            0
        );
    }, [paxCargoProperty, routeID, cargoProperties]);
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <Animated.View style={{ paddingHorizontal: 15, transform: [{ translateX: tripsAnimation }], position: 'absolute', width, top: 20, }}>
                {formLoading == true ? (
                    ...
                    <View>
                        ...
                        {paxCargoProperty.length > 1 && (
                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 18, gap: 8, alignSelf: 'flex-end' }}>
                                ...
                                <View style={{ borderBottomColor: '#cf2a3a', borderBottomWidth: 2, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 8 }}>
                        ...
                        <View style={{ height: height - 350, marginTop: 5 }}>
                            <ScrollView nestedScrollEnabled={true}>
                                                ...
                                                <View style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', borderColor: '#B3B3B3', paddingHorizontal: 5, borderWidth: 1, borderRadius: 5 }}>
                                                        <TouchableOpacity disabled={c.quantity == 1} onPress={() => handleCargoQuantity('minus', c.id)} style={{ paddingRight: 5 }}>
                                                            <Ionicons name={'remove'} size={18} color={c.quantity == 1 && "#e7e7e7ff"} />
                                                        </TouchableOpacity>
                                                        ...
                                                        <TouchableOpacity onPress={() => handleCargoQuantity('sum', c.id)} style={{ paddingLeft: 5 }}>
                                            ...
                                            <>
                                                { c?.cargoType == 'Rolling Cargo' ? (
                                                    ...
                                                    <View style={{ marginTop: 5 }}>
                                                        ...
                                                        <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                            <Dropdown onChange={(item) => {updatePaxCargoProperty(c.id, 'parcelCategory', item.label), updatePaxCargoProperty(c.id, 'parcelCategoryID', item.value)}} value={c.parcelCategoryID} data={cargoProperties?.data.parcel_categories.map((category: any) => ({ label: category.name.slice(1, -1), value: category.id })) } labelField="label" valueField="value" placeholder="Select Category" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                                containerStyle={{
                                                                    alignSelf: 'flex-start',
                                                                    width: '85%',
                                                                }}
                                ...
                                <View style={{ borderColor: '#B3B3B3', backgroundColor: '#fff', padding: 10, borderRadius: 8, borderWidth: 1, marginTop: 10 }}>
                                    ...
                                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5, height: 60 }}>
                                        <TextInput onChangeText={(text) => updateAmountAndNote('note', text)} placeholder='(Optional)' style={{ fontSize: 13 }} />
                                    </View>
                                </View>
