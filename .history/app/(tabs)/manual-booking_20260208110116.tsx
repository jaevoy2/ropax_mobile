...
export default function ManualBooking() {
    ...
    const handleTimeChecker = () => {
        ...
        const updatedTrips = trips?.map(trip => {
            ...
            if(dateTime > tripTime && trip.hasDeparted == false && trip.specific_days == dateTime.toISOString().split("T")[0]) {
        ...
        try {
            ...
            if(tripsFetch) {
                const tripsData: TripProps[] = tripsFetch.data.map((t: any) => ({
                    ...
                    specific_days: t.trip.specific_days,
                            ...
                            {contentLoading == true ? (
                                ...
                                <>
                                    ...
                                    {trips.some(t => t.hasDeparted == true) && (
                                        <>
                                            <Text style={{ color: '#7A7A85', marginTop: 20, fontWeight: 'bold' }}>Departed Trips</Text>
                                            {trips && trips.filter(t => t.hasDeparted == true).map((trip) => (
                                                <View key={trip.trip_id} style={{ paddingHorizontal: 15, paddingVertical: 20, backgroundColor: '#a7a7a7', borderRadius: 10, marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                                    <View>
                                                        <Text style={{ fontWeight: 'bold', fontSize: 13, color: '#e48891' }}>{`${trip.departure}`}</Text>
                                                        <Text style={{ fontWeight: 'bold', fontSize: 13, color: '#b4b4b4' }}>{`${trip.route_origin}  >  ${trip.route_destination} [ ${trip.vessel} ]`}</Text>
                                                    </View>
                                                    <Ionicons name="chevron-forward" color={'#b4b4b4'} size={18} />
                                                </View>
                                            ))}
                                        </>
                                    )}
