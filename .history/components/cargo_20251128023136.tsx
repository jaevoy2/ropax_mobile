...
export default function CargoComponent() {
    ...
    const cargoFormRef = useRef<BottomSheet>(null);
    const cargoFormSnapPoints = ['90%'];
    ...
    const handleSaveTrip = (selectedTrip: string, trip_id: number, origin: string, destination: string, mobileCode: string, code: string, web_code: string, departureTime: string, vesselID: number) => {
        setFormLoading(true);
        cargoFormRef.current?.snapToIndex(0);
        setTimeout(() => {
    ...
    return (
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={{ height: height }}>
                ...
                <BottomSheet ref={cargoFormRef} index={-1} snapPoints={cargoFormSnapPoints} bottomInset={1}>
                    <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: 10, paddingVertical: 10, backgroundColor: '#fff' }} showsVerticalScrollIndicator={false}>
                        <View>
                            {formLoading == true ? (
