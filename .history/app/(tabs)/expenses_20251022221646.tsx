    ...
    const fetchExpenses = async (monthName: string, year: string, fullDate?: string) => {
        ...
        if(expensesfetch) {
            const expense: ExpenseProps[] = expensesfetch.expenses.map((e: any) => ({
                ...
                date: (() => {
                    const formattedDate = new Date(e.created_at).toLocaleDateString('en-US', {
                        month: 'long',
                        day: 'numeric',
                        year: 'numeric',
                        timeZone: 'Asia/Manila'
            ...
            <View style={{ paddingVertical: 15 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomColor: '#CF2A3A', borderBottomWidth: 1, paddingBottom: 10, paddingHorizontal: 15 }}>
                    ...
                    <View style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                        ...
                        <Text style={{ color: '#CF2A3A', fontSize: 13, fontWeight: '900' }}>5000</Text>
                    </View>
                </View>
                ...
                <View style={{ height: height - 215 }}>
                    <ScrollView>
                        {fetchedExpenses.map((e) => (
                            <TouchableOpacity key={e.id} style={{ paddingHorizontal: 15, paddingVertical: 15, backgroundColor: '#f3f3f3ff', borderBottomColor: '#FFC1C5', borderBottomWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View>
