...
export default function ManualBooking() {
    const { trip, setTrip, setID, setOrigin, setDestination, setVesselID, setCode, setWebCode } = useTrip();
    ...
    const handleSaveTrip = (selectedTrip: string, trip_id: number, origin: string, destination: string, code: string, web_code: string) => {
        setLoading(true);            
        setTimeout(() => {
            ...
            <View style={{ padding: 20 }}>
                ...
                <View>
                    {contentLoading == true ? (
                        ...
                        <>
                        {trips?.map((trip) => (
                            <TouchableOpacity onPress={() => handleSaveTrip(trip.vessel, trip.trip_id, trip.route_origin, trip.route_destination, trip.code, trip.web_code)} key={trip.trip_id} style={{ paddingHorizontal: 15, paddingVertical: 25, backgroundColor: '#fff', borderRadius: 10, marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
