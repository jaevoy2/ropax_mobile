...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, letter, skip = 0, count = 0, onSeatSelect, type, accomm_id, bookedSeats }) => {
    ...
    useEffect(() => {
        const listen = channel.on('broadcast', { event: 'seat_select' }, payload => {
            ...
            setSeatSelectionChannel((prev) => [...prev, payload.paylaod.seat])
        });
        ...
        console.log(seatSelectionChannel)
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => {
                ...
                return(
                    ...
                    backgroundColor: booked ? booked.station.color : isPassenger ? '#BA68C8' : touristSeat.includes(`${letter}${seat}`) ? '#E6E2C6' : seatSelectionChannel.includes(`${letter}${seat}`) ? '#F9F9F9' : 'transparent',
