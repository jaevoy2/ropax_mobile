...
export default function SeatPlan() {
    ...
    useEffect(() => {
        const totolFare = passengers.reduce((sum, p) => {
            ...
            if(p.hasCargo == true) {
                subtotal += Number(p.cargo?.cargoFare || 0);
            }
    ...
    const renderBottomSheetBackdrop = useCallback(
        (props: any) => (
            <BottomSheetBackdrop {...props} appearsOnIndex={6} disappearsOnIndex={0} opacity={0.5} />
        ), []
    )
    ...
    return (
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={{ height: '100%', overflow: 'hidden' }}>
                ...
                <TouchableOpacity disabled={passengers.length > 0} onPress={() => handlePassesForm()} style={{ position: 'absolute', right: 20, top: 50, zIndex: 1 }}>
                ...
                {!passengers.some((p) => p.passType == 'Passes') && passengers.some((p) => p.seatNumber != null) && (
                    <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={passengers.length > 0 ? 0 : -1} bottomInset={1} backgroundComponent={renderBottomSheetBackdrop} enableHandlePanningGesture={false} enableContentPanningGesture={false}  handleIndicatorStyle={{ display: 'none' }} >
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, borderColor: '#B3B3B3', borderWidth: 1, backgroundColor: '#fff', borderRadius: 8, padding: 8, width: '100%', marginTop: 5 }}>
                                {passengers.map((p) => (
                                    <View key={p.seatNumber} style={{ position: 'relative' }}>
                                        ...
                                        <TouchableOpacity style={{ borderColor: errorForm.includes(p.seatNumber!) ? '#cf2a3a' : p.hasCargo == true && !errorForm.includes(p.seatNumber!) ? '#FFC107' : '#000', backgroundColor: errorForm.includes(p.seatNumber!) ? '#cf2a3b3d' : p.hasCargo ==  true && !errorForm.includes(p.seatNumber!) ? '#ffc1073b' : 'transparent', borderWidth: 1, borderRadius: 5, width: 45, height: 45, justifyContent: 'center' }}>
                                            <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 16 }}>{p.seatNumber}</Text>
                                        </TouchableOpacity>
                                    </View>
                                ))}
                            </View>
                            {passengers.some(p => p.hasCargo == true) && passengers.length > 1 && (
                                <Text style={{ color: '#4d4d4dff', fontSize: 12 }}>Note: The first seat is reserved for the passenger with cargo.</Text>
                            )}
                        </View>
                        {isFormVisible == true && (
