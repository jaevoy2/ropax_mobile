                        ...
                        <View style={{ gap: 5 }}>
                            {passengers.map((p, index) => (
                                <View key={`${p.accommodationID}-${index}`} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginLeft: 15 }}>
                                    <Text style={{ fontSize: 12, color: '#4b4b4bff', width: '40%', fontWeight: '400' }}>{`${p.name?.split(',')[1]?.trim().charAt(0)}. ${p.name?.split(',')[0]}`}</Text>
                                    <Text style={{ fontSize: 14, fontWeight: 'bold' }}>₱ {p.fare ? `${p.fare.toLocaleString('en-PH', { minimumFractionDigits: 2,  maximumFractionDigits: 2 })}.00` : '0.00'}</Text>
                                </View>
                            ))}
                        </View>
                        <View style={{ gap: 5 }}>
                ...
                {(passengers.some(p => p.hasCargo == true) || paxCargoProperty) && (
                    <View style={{ borderBottomColor: '#c9c9c9ff', borderBottomWidth: 1, paddingVertical: 10 }}>
                        <View style={{ width: '100%', flexDirection: 'column' }}>
                            <Text style={{ fontSize: 14, fontWeight: '500', flexDirection: 'column' }}>Cargo</Text>
