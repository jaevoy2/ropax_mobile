...
export default function SeatPlan() {
    ...
    const [continerHeight,  setContainerHight] = useState(0);
    ...
    useEffect(() => {
        if(!isFormVisible && passengers.length > 0) {
            const targetIndex = seatSnapPoints.findIndex((index) => {
                const value = Number(index.replace('%', '')) / 100 * height;
                return value >= continerHeight;
            });

            if(targetIndex !== -1 && targetIndex !== currentSnapPoints) {
