        ...
        <View style={{ backgroundColor: '#f1f1f1' }}>
            ...
            <View style={{ position: 'relative' }}>
                <View style={{ backgroundColor: '#fff', position: 'absolute', top: -70, left: '50%', transform: [{ translateX: '-50%' }], width: '85%', height: 400, borderRadius: 10, padding: 10 }}>
                    ...
                    <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
                        <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                            {trip.split(' ')?.[0] == 'HILONGOS' ? (
                                <Text style={{ fontSize: 25, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[0].charAt(0)}${trip.split(' ')[0].charAt(1)}${trip.split(' ')[0].charAt(2)}`}</Text>
                            ) : (
                                <Text style={{ fontSize: 25, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[0].charAt(0)}${trip.split(' ')[0].charAt(1)}${trip.split(' ')[0].charAt(3)}`}</Text>
                            )}
                            <Text style={{ fontSize: 10, color: '#cf2a3a' }}>{trip.split(' ')[0]}</Text>
                        </View>
                        <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                            {trip.split(' ')?.[4] == 'HILONGOS' ? (
                                <Text style={{ fontSize: 25, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[4].charAt(0)}${trip.split(' ')[4].charAt(1)}${trip.split(' ')[4].charAt(2)}`}</Text>
                            ) : (
                                <Text style={{ fontSize: 25, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[4].charAt(0)}${trip.split(' ')[4].charAt(1)}${trip.split(' ')[4].charAt(3)}`}</Text>
                            )}
                            <Text style={{ fontSize: 10, color: '#cf2a3a' }}>{trip.split(' ')[4]}</Text>
                        </View>
                    </View>
                </View>
            </View>
