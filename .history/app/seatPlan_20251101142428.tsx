...
export default function SeatPlan() {
    ...
    const [currentSnapPoints, setCurrentSnapPoints] = useState<number | null>(null);
    ...
    const seatSnapPoints = useMemo(() => {
    if (containerHeight === 0) return ["26%"]; // default small snap
    const containerPercent = (containerHeight / height) * 100;

    return [
        Math.min(containerPercent, 100) + "%", // fits container
        Math.min(containerPercent + 10, 100) + "%", // extra padding
        "90%", // max
    ];
}, [containerHeight]);

// Snap sheet dynamically when container height changes
useEffect(() => {
    if(!isFormVisible && containerHeight > 0) {
        const targetIndex = 0; // first snap point fits container
        if(targetIndex !== currentSnapPoints) {
            seatSheetRef.current?.snapToIndex(targetIndex);
            setCurrentSnapPoints(targetIndex);
        }
    }
}, [seatSnapPoints, containerHeight]);

// Example for handleSeatSelect
const handleSeatSelect = () => {
    if(currentSnapPoints == null && containerHeight > 0) {
        seatSheetRef.current?.snapToIndex(0); // snap to container-fitting point
        setCurrentSnapPoints(0);
    }
}
    ...
    const handleSeatChange = () => {
        ...
        seatSheetRef.current?.snapToIndex(currentSnapPoints!);
    }
            ...
            <View style={{ height: '100%', overflow: 'hidden' }}>
                ...
                <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={-1} bottomInset={1} enableContentPanningGesture={false} handleComponent={null}>
                    ...
                    <View style={{ paddingHorizontal: 10 }}>
                        ...
                        }} style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, borderColor: '#B3B3B3', borderWidth: 1, backgroundColor: '#fff', borderRadius: 8, padding: 8, width: '100%', marginTop: 5 }}>
                            {passengers.map((p) => (
