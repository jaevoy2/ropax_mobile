...
export default function L2Vessel({onSeatSelect}: L2VesselProps) {
    ...
    const channel = async () => {
        const { data } = await supabase.from('seat_channels').select('*').eq('trip_id', id).eq('event_type', 'selected');
        const selectedSeats = data?.map((d: any) => d.seat_number);
        setSeatSelectionChannel(selectedSeats || []);

        const listen = supabase.channel('seats_changes_channels').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'seat_channels' }, (payload) => {
            console.log(payload);
