...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    const ComputedCargoAmount = (cargo: CargoOnlyProperties) => {
        let amount: number;
        if(!cargo || !cargoProperties) amount = 0.00;

        if(cargo?.cargoType == 'Rolling Cargo') {
            if(!cargo.cargoBrandID  || !cargo.cargoSpecificationID) return amount = 0.00;
        ...
        if(cargo?.cargoType == 'Parcel') {
            if(!cargo.parcelCategoryID) amount = 0.00;
        ...
        setCargoAmountState(prev =>  [...prev, { id: cargo.id, cargoAmount: amount }]);
        return amount;
    };
