    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: '100%' }}>
            {items.map((seat) => (
                ...
                borderRadius: 3, backgroundColor: passengers.some((p) => p.seatNumber == Number(seat)) ? '#BA68C8' : touristSeat.includes(seat) ? '#E6E2C6' : 'transparent', justifyContent: 'center', margin: 2 }}>
...
export default function SRVessel({ onSeatSelect }: SRVesselProps) {
    ...
    return (
        <View style={{ width: width - 20, height: height + 290, backgroundColor: '#FAFAFA', marginTop: 20, paddingTop: 10, borderRadius: 50,  }}>
            ...
            <View style={{ marginTop: 25, width: '100%', gap: 5 }}>
                ...
                {/* <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', alignSelf: 'center', width: '100%' }}>
                    <View style={{ width: '46%', flexDirection: 'column', alignItems: 'center' }}>
                        <Text style={{ fontWeight: 'bold', textAlign: 'center', fontSize: 12, marginBottom: 4 }}>Senior/PWD</Text>
                        <SeatPlan start={1} limit={52} skipPattern={true} onSeatSelect={assignseat} />
                    </View>
                    <View style={{ width: '46%' }}>
