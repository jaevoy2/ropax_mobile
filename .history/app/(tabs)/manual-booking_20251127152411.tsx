        ...
        <View style={{ backgroundColor: '#f1f1f1', height: height, position: 'relative' }}>
            ...
            <ScrollView refreshControl={ <RefreshControl refreshing={refresh} onRefresh={handleRefresh} colors={['#cf2a3a']} /> } >
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', borderBottomColor: '#cf2a3a', borderBottomWidth: 1 }}>
                    {bookingTypes.map((bt) => (
                        <TouchableOpacity onPress={() => setBookingType(bt.type)} key={bt.type} style={{ flexDirection: 'row', alignItems: 'center', gap: 3, paddingVertical: 10, justifyContent: 'center', width: '50%', backgroundColor: bookingType == bt.type ? '#cf2a3a' : '#fff' }}>
                            <MaterialCommunityIcons name={bt.icon as any} style={{ color: bookingType == bt.type ? '#fff' : '#cf2a3a' }} size={18} />
                            <Text style={{ color: bookingType == bt.type ? '#fff' : '#cf2a3a' }}>{bt.type}</Text>
                        </TouchableOpacity>
                    ))}
                </View>
