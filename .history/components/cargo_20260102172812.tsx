...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    const { routeID, setRouteID, setVessel, setID, setOrigin, setDestination, setVesselID, setCode, setWebCode, setDepartureTime, setMobileCode, clearTrip } = useTrip();
    ...
    const closeFormSheet = () => {
        ...
        setTotalAndNote({ totalAmount: 0, note: '' })
        setFormId(0);
        setFormLoading(true);
        Animated.timing(tripsAnimation, {
            toValue: width,
    ...
    const handleSaveCargo = () => {
        ...
        setTimeout(() => {
            ...
            if(totalAndNote.totalAmount == null) {
    ...
    const ComputedCargoAmount = (cargo: PaxCargoProperties) => {
        let prop: any;
        if(!cargo || !cargoProperties) return { amount: 0.00, optionID: 0 };

        if(cargo?.cargoType == 'Rolling Cargo') {
            if(!cargo.cargoBrandID  || !cargo.cargoSpecificationID) return { amount: 0.00, optionID: 0 };;
            ...
            return {
                amount: Number(prop?.price ?? 0.00 * cargo.quantity),
                optionID: prop.id
            }
        }

        if(cargo?.cargoType == 'Parcel') {
            if(!cargo.parcelCategoryID) return { amount: 0.00, optionID: 0 };;
        ...
        if(cargo?.cargoType == 'Animal/Pet') {
            cargoProperties.data.cargo_options?.find(
    ...
    useEffect(() => {
        paxCargoProperty.map((c) => {
            const prop = ComputedCargoAmount(c);

            if(c.cargoAmount !== prop.amount) {
                updatePaxCargoProperty(c.id, 'cargoOptionID', prop.optionID);
                updatePaxCargoProperty(c.id, 'cargoAmount', prop.amount)
            }
        });

        const total = paxCargoProperty.reduce(
            (sum, c) => sum + Number(ComputedCargoAmount(c)),
            0
        );
        updateAmountAndNote('totalAmount', total);
    }, [paxCargoProperty, routeID, cargoProperties])
                            ...
                            <ScrollView nestedScrollEnabled={true}>
                                            ...
                                            <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                                                ...
                                                <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 8 }}>
                                                    ...
                                                    <Text style={{ fontWeight: 'bold', textAlign: 'right', fontSize: 16 }}>
                                                        {Number(ComputedCargoAmount(c.amount)).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </Text>
                                                </View>
                                            </View>
                                ...
                                <View style={{ borderColor: '#B3B3B3', backgroundColor: '#fff', padding: 10, borderRadius: 8, borderWidth: 1, marginTop: 10 }}>
                                    ...
                                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5, height: 60 }}>
                                        <TextInput onChangeText={(text) => updateAmountAndNote('note', text)} placeholder='Receiver, e.g' style={{ fontSize: 13 }} />
                                    </View>
                                </View>
