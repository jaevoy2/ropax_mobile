...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, letter, skip, count }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
            {items.map((seat) => (
                <TouchableOpacity key={seat} style={{ width: 35, height: 35, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: touristSeat.includes(seat) ? '#E6E2C6' : 'transparent', justifyContent: 'center', margin: 2 }}>
                    <Text style={{ fontSize: 11, fontWeight: 'bold', textAlign: 'right' }}>{`${letter}${seat}`} </Text>
                </TouchableOpacity>
            ))} 
        </View>
    )
