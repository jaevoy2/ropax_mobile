...
export default function SeatPlan() {
    ...
    useEffect(() => {
        if(!isFormVisible) {
            const newIndex = Math.min(Math.floor(passengers.length / 7), seatSnapPoints.length - 1);

            if(newIndex > currentSnapPoints!) {
                seatSheetRef.current?.snapToIndex(newIndex);
                setCurrentSnapPoints(newIndex);
            } 
        }
    }, [passengers]);
