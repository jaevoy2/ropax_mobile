...
export default function ManageBooking() {
    ...
    useEffect(() => {
        ...
        const PHTimezone = date.toLocaleDateString('en-CA', { timeZone: 'Asia/Manila' })

        setDate(PHTimezone)
        setFormattedDate(date.toLocaleDateString('en-US', options));
        fetchBooking(PHTimezone, null);

        if(PHTimezone != date.toISOString().split('T')[0] || searchValue.length > 0) {
            const requestInterval = setInterval(() => fetchBooking(PHTimezone, null), 2000);
