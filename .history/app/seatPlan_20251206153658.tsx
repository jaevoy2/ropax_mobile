...
export default function SeatPlan() {
    ...
    useEffect(() => {
        ...
        const date = new Date();
        setYear(date.getFullYear().toString().slice(-2));3
    }, []);
    ...
    const handleSeatSelect = () => {
        if(passengers.length == 0 || (passengers.length == 1 && passengers[0].seatNumber == '')) {
    ...
    const handleRemoveSeat = (seat: number | string | undefined) => {
        ...
        if(hasCargo && paxCargoSeat?.seatNumber == seat) {
            setPassengers(prev => 
                prev.map(p => p.hasCargo ? { ...p, seatNumber: '', accommodation: '', accommodationID: null }: p)
            );
        }

        if(newSeats.length == 0 || paxCargoSeat?.seatNumber == '') {
    ...
    return (
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={{ height: '100%', overflow: 'hidden' }}>
                ...
                {!passengers.some((p) => p.passType == 'Passes') && passengers.some((p) => p.seatNumber != '') && (
                    <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={passengers.length > 0 || passengers.find((p) => p.hasCargo )?.seatNumber != '' ? 0 : -1} bottomInset={1} onChange={(index) => handleSheetChange(index)}>
