...
export default function Forms({ errorForm }: FormProps) {
    ...
    useEffect(() => {
        computedCargoList.forEach((c) => {
            const passenger = passengers.filter((p, index) => p.seatNumber == c.seatNumber || index == c.paxIndex);
            if(!passenger) return;

            const prevCargoProp = passenger.find(p => 
                p.cargo.find((cargo, index) => index == c.cargoIndex)
            )

            console.log('prev:', passenger[0].cargo.length)
                
            if(!prevCargoProp) return;

            // if(prevCargoProp.cargo. != c.cargoAmount || prevCargoProp.cargoOptionID != c.cargoOptionID) {
