                ...
                <ScrollView>
                    {passengers.map((p) => (
                        <View key={p.seatNumber} style={{ borderColor: errorForm == p.seatNumber ? '#cf2a3a' : '#B3B3B3', borderWidth: 1, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 10, marginBottom: 20, backgroundColor: '#fff' }}>
                            ...
                            {p.hasInfant && (
                                <>
                                    <TouchableOpacity onPress={() => addInfant(p.seatNumber!, {name: '', gender: '', age: 0})} style={{ borderColor: '#cf2a3a', borderWidth: 1, padding: 4, borderRadius: 5, alignSelf: 'flex-end', marginTop: 10, marginBottom: -15, flexDirection: 'row', gap: 3, alignItems: 'center' }}>
                                        <Text style={{ color: '#cf2a3a', fontWeight: 600 }}>Add Infant</Text>
                                        <Ionicons name={'add-circle'} size={25} color={'#cf2a3a'} />
                                    </TouchableOpacity>
