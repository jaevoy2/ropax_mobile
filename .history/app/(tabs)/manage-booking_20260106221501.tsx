    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <View style={{ justifyContent: 'center', paddingTop: 20, paddingHorizontal: 20 }}>
                ...
                <Text style={{ marginTop: 20, fontWeight: 'bold', fontSize: 20 }}>Bookings</Text>
                <FlatList data={bookings} keyExtractor={(booking) => booking.referenceNumber} style={{ gap: 10 }} renderItem={({ item }) => (
                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, backgroundColor: '#fff', borderRadius: 5, padding: 8 }}>
                        <Text style={{ fontWeight: 'bold', fontSize: 16 }}>{item.name}</Text>
                        <Text style={{ fontSize: 10, color: '#cf2a3a', fontWeight: 'bold' }}>{item.referenceNumber}</Text>
                    </View>
                )}/>
            </View>
        </View>
