...
export default function ManageBooking() {
    ...
    const fetchBooking = async (dateString: string) => {
        try {
            ...
            console.log(response.data[0].bookings.flatMap((b: any) => b.trip_schedule.trip.route.mobile_code).mobile_code)
            if(!response.error) {
                const paxData: PaxBookingProps[] = response.data.map((passenger: any) => ({
                    id: passenger.id,
                    name: `${passenger.first_name} ${passenger.last_name}`,
                    departureDate: new Date(passenger.bookings.find((c: any) => c.created_at)).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
                    }),
                    departureTime: new Date(`1970-01-01T${passenger.bookings.flatMap((b: any) => b.trip_schedule.trip.departure_time)}`).toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit',
                        hour12: true
                    }),
                    route: passenger.bookings.flatMap((t: any) => t.trip_schedule.trip.route.mobile_code),
                    vessel: passenger.bookings.flatMap((v: any) => v.trip_schedule.trip.vessel.name),
