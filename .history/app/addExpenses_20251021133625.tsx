            ...
            <View style={{ padding: 10, marginTop: 20 }}>
                ...
                <View style={{ padding: 10, borderColor: '#cf2a3a', borderWidth: 1, borderRadius: 8 }}>
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 5, marginTop: 15, marginBottom: 5 }}>
                        {trips?.map((t) => (
                            <>
                                <TouchableOpacity onPress={() => setSelectedTrip(t.trip_id)} key={t.trip_id} style={{ backgroundColor: selectedTrip ? '#cf2a3a' : 'transparent', borderColor: '#cf2a3a', borderWidth: 1, paddingVertical: 4, paddingHorizontal: 5, borderRadius: 5  }}>
                                    <View>
                                        <Text style={{ fontWeight: 'bold', fontSize: 10, color: selectedTrip ? '#fff' : '#cf2a3a' }}>{`${t.departure}`}</Text>
                                        <Text style={{ fontWeight: 'bold', fontSize: 10, color: selectedTrip ? '#fff' : '#000' }}>{`${t.route_origin} > ${t.route_destination} [ ${t.vessel} ]`}</Text>
                                    </View>
                                </TouchableOpacity>
                            </>
                        ))}
