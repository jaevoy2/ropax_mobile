...
type CargoContextType = {
    ...
    updateCargoName: (name: string) => void;
}
...
export const CargoProvider = ({ children }: {children: ReactNode}) => {
    ...
    const updateCargoName = (name: string) => {
        setCargoOwner({name})
    }
    return (
        <CargoContext.Provider value={{ cargoProperties, setCargoProperties, cargoOwner, setCargoOwner, updateCargoName }}>
            {children}
