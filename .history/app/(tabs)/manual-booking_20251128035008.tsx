...
export default function ManualBooking() {
    const { vessel, setVessel, setID, setOrigin, setDestination, setVesselID, setCode, setWebCode, setDepartureTime, setMobileCode } = useTrip();
            ...
            <Animated.View style={{ height, position: 'absolute', bottom: 0, backgroundColor: '#fff', width: width, transform: [{ translateY }], borderTopRightRadius: 20, borderTopLeftRadius: 20 }}>
                <View style={{ padding: 10 }}>
                    {totalSheetLoading == true ? (
                        ...
                        <>
                            <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', borderBottomColor: '#8f8f8fff', borderBottomWidth: 1, paddingBottom: 5 }}>
                            ...
                            {trips && trips?.length > 0 ? (
                                <>
                                    <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center', flexWrap: 'wrap', paddingTop: 10 }}>
                                        {trips?.map((trip) => (
