...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, alignItems: 'center' }}>
            {items.map((seat) => (
                <TouchableOpacity key={seat} style={{  paddingVertical: 3, height: 35, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: touristSeat.includes(seat) ? '#E6E2C6' : 'transparent', alignItems: 'center', justifyContent: 'center', margin: 2 }}>
