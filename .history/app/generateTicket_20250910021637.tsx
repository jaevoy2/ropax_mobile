                    ...
                    <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
                        <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                            {trip.split(' ')?.[0] == 'HILONGOS' ? (
                                <Text style={{ fontSize: 20, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[0].charAt(0)}${trip.split(' ')[0].charAt(1)}${trip.split(' ')[0].charAt(2)}`}</Text>
                            ) : (
                                <Text style={{ fontSize: 20, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[0].charAt(0)}${trip.split(' ')[0].charAt(1)}${trip.split(' ')[0].charAt(3)}`}</Text>
                            )}
                            <Text style={{ fontSize: 12, color: '#cf2a3a' }}>{trip.split(' ')[0]}</Text>
                        </View>
                        <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                            {trip.split(' ')?.[4] == 'HILONGOS' ? (
                                <Text style={{ fontSize: 20, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[4].charAt(0)}${trip.split(' ')[4].charAt(1)}${trip.split(' ')[4].charAt(2)}`}</Text>
                            ) : (
                                <Text style={{ fontSize: 20, fontWeight: '900', color: '#cf2a3a' }}>{`${trip.split(' ')[4].charAt(0)}${trip.split(' ')[4].charAt(1)}${trip.split(' ')[4].charAt(3)}`}</Text>
                            )}
                            <Text style={{ fontSize: 12, color: '#cf2a3a' }}>{trip.split(' ')[4]}</Text>
                        </View>
                    </View>
                </View>
            </View>
