                    ...
                    <View style={{ paddingVertical: 5 }}>
                        ...
                        {passengers.map((p) => (
                            <View key={p.seatNumber} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <Text style={{ fontSize: 12, width: 100 }}>{`${p.name?.split(',')[0].charAt(0)}, ${p.name?.split(',')[0]}`}</Text>
                                <Text style={{ fontSize: 12, width: 50 }}>{p.passType?.charAt(0)}</Text>
                                <Text style={{ fontSize: 12, textAlign: 'center' }}>{`#${p.seatNumber}`}</Text>
                            </View>
                        ))}
                        {passengers.map((p, index) => p?.hasInfant && (
                            <View key={index}>
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <Text style={{ fontSize: 12, width: 100 }}>{`${p.name?.split(',')[1].charAt(0)}, ${p.name?.split(',')[0]}`}</Text>
                                    <Text style={{ fontSize: 12, width: 50 }}>{p.infantGender?.charAt(0)}</Text>
                                    <Text style={{ fontSize: 12, textAlign: 'center', backgroundColor: '#000' }}>{p.infantAge}</Text>
                                </View>
                            </View>
                        ))}
                    </View>
