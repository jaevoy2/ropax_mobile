...
export default function AddExpenses() {
    ...
    const addExpenseForm = () => {
        setExpenses(prev => [...prev, { trip_id: 0, description: '', amount: 0, categoryID: 0 }]);
    }

    const removeExpenseForm = (formIndex: number) => {
        const remainingForm = expenses.filter((e, index) => index != formIndex);
    ...
    return (
        <View style={{ backgroundColor: '#f1f1f1', position: 'relative', height: height, }}>
                    ...
                    {trips?.map((t) => (
                        <TouchableOpacity onPress={() => setSelectedTrip(t.trip_id)} key={t.trip_id} style={{ backgroundColor: selectedTrip == t.trip_id ? '#cf2a3a' : 'transparent', borderColor: selectedTrip == t.trip_id ? 'transparent' : '#cf2a3a', borderWidth: 1, paddingVertical: 6, paddingHorizontal: 5, borderRadius: 5  }}>
                            <View>
                                ...
                                <Text style={{ fontWeight: 'bold', fontSize: 11, color: selectedTrip == t.trip_id ? '#fff' : '#000' }}>{`${t.route_origin} > ${t.route_destination} [ ${t.vessel} ]`}</Text>
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>
                    ...
                    <View style={{ minHeight: height - 350, maxHeight: height - 300, marginTop: 10 }}>
                        <ScrollView style={{ maxHeight: height - 300 }} contentContainerStyle={{ minHeight: height - 350 }}>
                            {expenses.map((e, index) => (
                                <View key={index} style={{ padding: 10, marginTop: 15, borderColor: '#cf2a3a', borderWidth: 1, borderRadius: 8 }}>
                                    {index !== 0 && (
                                        <TouchableOpacity onPress={() => removeExpenseForm(index)}  style={{ alignSelf: 'flex-end' }} >
                                    ...
                                    <View>
                                        ...
                                        <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                            <TextInput onChangeText={(text) => updateExpense(index, 'description', text)} placeholder='e.g. Vessel Oil' style={{ fontSize: 13 }} />
                                        </View>
                                    </View>
                                    <View style={{ marginTop: 5, flexDirection: 'row', gap: 8, alignItems: 'flex-end' }}>
                                        <View style={{ width: '25%' }}>
                                            ...
                                            <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 5 }}>
                                                ...
                                                <TextInput onChangeText={(text) => updateExpense(index, 'amount', Number(text))} keyboardType='numeric' placeholder='00.00' style={{ fontSize: 13, textAlign: 'right', }} />
                                            </View>
                                        </View>
                                        <View style={{ width: '72.5%' }}>
                                            ...
                                            <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <Dropdown onChange={item => updateExpense(index, 'categoryID', item.value)} value={e.categoryID || undefined} data={dropdownCategory} labelField="label" valueField="id" placeholder="Select Category" style={{ height: 40, width: '80%', paddingHorizontal: 10 }}
                                                    containerStyle={{
                                                        alignSelf: 'flex-start',
                                                        width: '53%',
                                                    }}
                                                    selectedTextStyle={{ fontWeight: '500', fontSize: 13, lineHeight: 35, }}
                                                    renderRightIcon={() => (
            ...
            <TouchableOpacity style={{ width: '95%', paddingVertical: 13, borderRadius: 60, position: 'absolute', bottom: 0, backgroundColor: '#cf2a3a', transform: [{ translateX: '-50%' }], left: '50%' }}>
