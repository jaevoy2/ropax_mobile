...
export default function Forms({ errorForm }: FormProps) {
    ...
    const handlePaxFare = (passenger: PassengerProps, index: number) => {
        const prop = paxFares?.find(
            p => 
                p.routes_id == routeID &&
                p.vessel_id == vessel_id &&
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                                ...
                                <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                    ...
                                    <TextInput onChangeText={(text) => onFareInput(index, p.seatNumber, text)} value={String(p?.fare?.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }))} keyboardType={'numeric'} placeholder='00.00' style={{ fontWeight: 'bold', textAlign: 'right' }} />
                                </View>
                            </View>
                        </View>
                        {!passengers.some((p) => p.passType == 'Passes') && (
                                    ...
                                    {typeLoading == true ? (
                                        ...
                                        <>
                                            {passengerType?.filter((t) => t.name != 'Infant' && t.name != 'Passes')
                                                .map((type) => (
                                                <TouchableOpacity onPress={() => {handlePaxFare(p, index), updatePassenger(index, p.seatNumber, 'passType', type.name), updatePassenger(index, p.seatNumber, 'passType_id', type.id), updatePassenger(index, p.seatNumber!, 'passTypeCode', type.code)}}
