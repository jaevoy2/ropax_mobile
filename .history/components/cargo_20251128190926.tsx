...
export default function CargoComponent() {
    ...
    const handleSaveTrip = (vesselName: string, trip_id: number, origin: string, destination: string, mobileCode: string, code: string, web_code: string, departureTime: string, vesselID: number) => {
        ...
        setTimeWithRoute(`${departure_time} | ${mobileCode}`)
        toggleFormSheet();
        setTimeout(() => {
    ...
    return (
        <View style={{ height: height }}>
            ...
            {formExpanded && (
                <>
                    <Animated.View style={{ opacity: sheetFadeInAnim, position: 'absolute', top: -150 }}>
            ...
            <Animated.View style={{ height, position: 'absolute', bottom: 0, backgroundColor: '#fff', width: width, transform: [{ translateY: sheetTranslateY }], borderTopRightRadius: 20, borderTopLeftRadius: 20 }}>
                <View style={{ padding: 10 }}>
                    {formLoading == true ? (
                        ...
                        <View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', borderBottomColor: '#8f8f8fff', borderBottomWidth: 1, paddingBottom: 10 }}>
                                <View>
                                    <Text style={{ color: '#cf2a3a', fontSize: 20, fontWeight: 'bold', letterSpacing: 1 }}>{selectedVessel}</Text>
                                    <Text style={{ color: '#747373ff', fontSize: 10 }}>{timeWithRoute}</Text>
                                </View>
                                <TouchableOpacity onPress={() => closeFormSheet()}>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 10, width: '100%', justifyContent: 'space-between' }}>
                                {tripTypes.map((type) => (
                                    <TouchableOpacity onPress={() => setTripType(type.name)} key={type.name} style={{ paddingVertical: 8, width: '49%', borderColor: '#cf2a3a', borderWidth: 1, borderRadius: 3, backgroundColor: tripType == type.name ? '#cf2a3a' : 'transparent' }}>
                                        <Text style={{ color: tripType == type.name ? '#fff' : '#cf2a3a', textAlign: 'center', fontWeight: 'bold' }}>{type.name}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </View>
