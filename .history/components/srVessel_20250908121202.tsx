...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, onSeatSelect }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
            {items.map((seat) => (
                <TouchableOpacity onPress={() => onSeatSelect?.(seat)} key={seat} style={{  paddingVertical: 2, width: 32, height: 32, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: touristSeat.includes(seat) ? '#E6E2C6' : 'transparent', alignItems: 'center', justifyContent: 'center', margin: 2 }}>
...
export default function SRVessel({ onSeatSelect }: SRVesselProps) {
    ...
    return (
        <View style={{ width: 330, height: height + 290, backgroundColor: '#fff', marginTop: 20, paddingTop: 10, paddingBottom: 50, borderRadius: 50 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 10, width: '80%', alignSelf: 'center' }}>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 90, gap: 4, justifyContent: 'center' }}>
                    <Text style={{ fontWeight: 'bold', textAlign: 'center', fontSize: 12, marginBottom: 4 }}>Senior/PW</Text>
