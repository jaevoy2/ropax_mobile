...
type CargoContextType = {
    ...
    setCargoOwner: React.Dispatch<React.SetStateAction<CargoOwner | null>>
    updateCargoOwner: (name: string) => void;
}
...
export const CargoProvider = ({ children }: {children: ReactNode}) => {
    ...
    const [cargoOwner, setCargoOwner] = useState<CargoOwner | null>(null);

    const updateCargoOwner = (name: string) => {
        setCargoOwner({name})
    }
    return (
        <CargoContext.Provider value={{ cargoProperties, setCargoProperties, cargoOwner, setCargoOwner, updateCargoOwner }}>
            {children}
