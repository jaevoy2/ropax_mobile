...
export default function SeatPlan() {
    ...
    const handleSeatSelect = () => {
        if(passengers.length == 0 || (passengers.length == 1 && passengers[0].hasCargo == true)) {
        ...
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={{ height: '100%', overflow: 'hidden' }}>
                ...
                {!passengers.some((p) => p.passType == 'Passes') && passengers.some((p) => p.seatNumber != null) && (
                    <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={passengers.length > 0 || passengers[0].seatNumber == '' ? 0 : -1} bottomInset={1} onChange={(index) => handleSheetChange(index)}>
