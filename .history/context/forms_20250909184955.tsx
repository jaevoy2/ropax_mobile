...
export default function Forms() {
    ...
    const addInfant = (seatNum: number | string, InfantAddress: string, InfantNationality: string ) => {
        setTimeout(() => {
            setPassengers(prev => [...prev, { seatNumber: seatNum, address: InfantAddress, nationality: InfantNationality }])
        }, 500);
                ...
                <ScrollView>
                    {passengers.map((p) => p.passType != 'Infant' && (
                        <View key={p.seatNumber} style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 10, marginBottom: 30 }}>
                            ...
                            <TouchableOpacity onPress={() => {updatePassenger(p.seatNumber!, 'hasInfant', true), addInfant(p.seatNumber!, p.address!, p.nationality!)}} style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15 }}>
