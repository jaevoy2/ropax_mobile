    ...
    const seats = useMemo(() => {
        ...
        if (!skipPattern) {
            ...
            for (let i = start; i <= limit;) {
                for (let j = 1; j <= 4; j++, i++) arr.push(i);
                i += 4;
            }
        }
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => (
                ...
                backgroundColor: bookedSeats?.find((s) => s.seat_no == seat) ? bookedSeats.find((s) => s.seat_no == seat)!.station.color
                ...
                alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                    <Text style={{ fontSize: 14, fontWeight: 'bold', 
                        color: bookedSeats?.find((s) => s.seat_no == seat) ? '#fff' : passengers.some((p) => p.seatNumber == Number(seat)) ? '#fff' : '#000' }}>
                        {bookedSeats?.find((s) => s.seat_no == seat)?.passTypeCode ?? seat}
                    </Text>
                </TouchableOpacity>
            ))} 
        </View>
