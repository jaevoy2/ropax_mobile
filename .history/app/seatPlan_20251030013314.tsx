...
export default function SeatPlan() {
    ...
    const seatSnapPoints = useMemo(() => ["40%", "90%"], []);
    const passesSnapPoints = useMemo(() => ["90%"], []);
    const initialSnapPoints = useMemo(() => ['CONTENT_HEIGHT'] as (string | number)[], []);
...
>
  ...
  <View style={{ flexDirection:'row', justifyContent: 'space-between', alignItems: 'center', padding: 10 }}>
    <Text style={{ fontWeight: 'bold' }}>Seat# selected</Text>
    <TouchableOpacity onPress={handleSeatChange} style={{ flexDirection: 'row', alignItems: 'center' }}>
  ...
  <BottomSheetScrollView
    onLayout={handleContentLayout} // Needed for dynamic snap points
  ...
  >
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, padding: 8 }}>
      {passengers.map(p => (
        <View key={p.seatNumber} style={{ position: 'relative' }}>
          <TouchableOpacity onPress={() => handleRemoveSeat(p.seatNumber)} style={{ position: 'absolute', top: -8, right: -4 }}>
            <Ionicons name="remove-circle" size={20} color="#cf2a3a" />
          </TouchableOpacity>
          <View style={{ borderColor: '#000', borderWidth: 1, borderRadius: 5, width: 45, height: 45, justifyContent: 'center' }}>
            <Text style={{ textAlign: 'center', fontWeight: 'bold' }}>{p.seatNumber}</Text>
          </View>
        </View>
      ))}
    </View>
    ...
    {isFormVisible && (
      <View style={{ paddingVertical: 10 }}>
        {formLoading ? (
          ...
          <View>
            <Forms errorForm={errorForm} /> {/* Wrap Forms inside a View */}
          </View>
        )}
      </View>
    )}
    ...
    <TouchableOpacity
      onPress={isFormVisible ? handleSave : onFormView}
      style={{ backgroundColor: '#cf2a3a', width: '95%', alignSelf: 'center', borderRadius: 30, paddingVertical: 15 }}
    >
      {saveloading ? <ActivityIndicator size="small" color="#fff" /> : <Text style={{ color: '#fff', fontWeight: 'bold', textAlign: 'center' }}>{isFormVisible ? 'Proceed Payment' : 'Continue'}</Text>}
    </TouchableOpacity>
  </BottomSheetScrollView>
</BottomSheet>
