...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, letter, skip = 0, count = 0, onSeatSelect, type, accomm_id, bookedSeats }) => {
    ...
    useEffect(() => {
        console.log('seats selected', seatSelectionChannel);

        const listen = supabase.channel('seat_selection').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'seats_channels' }, payload => {
