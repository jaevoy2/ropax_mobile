...
type CargoContextType = {
    totalAndNote: CargoTotalAndNote | null;
    cargoProperties: CargoProperties | null;
    cargoOnlyProperty: PaxCargoProperties[];
    setCargoProperties: React.Dispatch<React.SetStateAction<CargoProperties | null>>;
    setCargoOnlyProperty: React.Dispatch<React.SetStateAction<PaxCargoProperties[]>>
    setTotalAndNote: React.Dispatch<SetStateAction<CargoTotalAndNote | null>>

    updateCargoOnlyProperty: <K extends keyof PaxCargoProperties> (
        indentifier: number,
        key: K,
        value: PaxCargoProperties[K]
    ) => void;

    updateAmountAndNote: <K extends keyof CargoTotalAndNote> (
        key: K,
        value: CargoTotalAndNote[K]
    ) => void;
}
...
export const CargoProvider = ({ children }: {children: ReactNode}) => {
    ...
    const [cargoOnlyProperty, setCargoOnlyProperty] = useState<PaxCargoProperties[]>([]);
    const [totalAndNote, setTotalAndNote] = useState<CargoTotalAndNote | null>(null)

    const updateCargoOnlyProperty = <K extends keyof PaxCargoProperties>(
        indentifier: number,
        key: K,
        value: PaxCargoProperties[K]
    ) => {
    ...
    const updateAmountAndNote = <K extends keyof CargoTotalAndNote> (
        key: K,
        value: CargoTotalAndNote[K]
    ) => {
        setTotalAndNote(prev => ({ ...prev, [key]: value }))
    }
    
    return (
        <CargoContext.Provider value={{ totalAndNote, cargoProperties, cargoOnlyProperty, updateAmountAndNote, setTotalAndNote, setCargoProperties, setCargoOnlyProperty, updateCargoOnlyProperty }}>
            {children}
