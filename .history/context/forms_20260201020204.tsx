...
export default function Forms({ errorForm }: FormProps) {
    ...
    const getPassengerFare = async (passType_id: number, accomm_id: number, seatNum: string | number) => {
        try{
            const passFare = await FetchFares(passType_id, accomm_id, vessel_id, routeID);
            console.log(passFare)

            if(!passFare.error) {
                const fare = passFare.data.fare;
                updatePassenger(null, seatNum, 'fare', fare);
            }
        }catch(error: any) {
            Alert.alert('Error', error.message);
        }
    ...
    const handleGetPaxFare = (passenger: PassengerProps) => {
        let prop: any;

        if(!passenger || !paxFares) return Number(0) ;

        prop = paxFares?.find(
            p => 
                p.routes_id == routeID &&
                p.vessl_id == vessel_id &&
                p.accommodation_type_id == passenger.accommodationID&&
                p.passenger_type_id == passenger.passType_id
        );

        console.log('lol', prop.fare)
        if(!prop) return Number(0);
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                                ...
                                <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                    ...
                                    <TextInput onChangeText={(text) => onFareInput(index, p.seatNumber, text)} value={String(handleGetPaxFare(p)?.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }))} keyboardType={'numeric'} placeholder='00.00' style={{ fontWeight: 'bold', textAlign: 'right' }} />
                                </View>
                            </View>
                        </View>
                        {!passengers.some((p) => p.passType == 'Passes') && (
                                    ...
                                    {typeLoading == true ? (
                                        ...
                                        <>
                                            {passengerType?.filter((t) => t.name != 'Infant' && t.name != 'Passes')
                                                .map((type) => (
                                                <TouchableOpacity onPress={() => {getPassengerFare(type.id, p.accommodationID!, p.seatNumber!), updatePassenger(index, p.seatNumber, 'passType', type.name), updatePassenger(index, p.seatNumber, 'passType_id', type.id), updatePassenger(index, p.seatNumber!, 'passTypeCode', type.code)}}
