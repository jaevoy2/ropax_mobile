...
import { Alert, Dimensions, Image, NativeModules, Platform, ScrollView, Share, Text, TouchableOpacity, View } from "react-native";
...
export default function TicketGenerator() {
    ...
    const generateTicket = async () => {
        ...
        try {
            ...
            const uri = await FileProviderModule.getUri(
            ...
            await Share.share({
                url: uri,
                title: "Print Ticket",
                message: "Send to RawBT for printing",
            });
            } else {
            // iOS fallback — share file directly
            await Share.share({
                url: cacheUri,
                title: "Print Ticket",
                                    ...
                                    {p.infant?.flatMap((i, index) => (
                                        <View key={`${p.seatNumber}-${index}`} style={{ marginBottom: 3 }}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                                ...
                                                <Text style={{ fontSize: 12, width: '21%', textAlign: 'right' }}>₱00.00</Text>
                                            </View>
                                        </View>
                                    ))}
                                </>
