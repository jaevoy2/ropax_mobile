...
export default function SeatPlan() {
    ...
    useEffect(() => {
        if(!isFormVisible && passengers.length > 0) {
            if(passengers.length % 7 === 0) {
                const nextIndex = Math.min((currentSnapPoints ?? 0) + 1, seatSnapPoints.length - 1);
                
                if(nextIndex > (currentSnapPoints ?? 0)) {
                    seatSheetRef.current?.snapToIndex(nextIndex);
                    setCurrentSnapPoints(nextIndex);
                    console.log(nextIndex, currentSnapPoints, seatSnapPoints.length)
                }
            }
        }
    }, [passengers]);
