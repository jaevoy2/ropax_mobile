...
export default function ManualBooking() {
    ...
    const handleFetchTotalBookings = async (trip_id: number) => {
        try {
                    ...
                    {totalSheetLoading == false ? (
                        ...
                        <>
                            ...
                            <View style={{ flexDirection: 'row', gap: 5, alignItems: 'center', flexWrap: 'wrap', marginTop: 10 }}>
                                {trips?.map((trip) => (
                                    <TouchableOpacity key={trip.trip_id} style={{ paddingHorizontal: 18, paddingVertical: 5, backgroundColor: '#fff', borderRadius: 5, marginTop: 12, flexDirection: 'row', alignItems: 'center', borderColor: '#cf2a3a', borderWidth: 1 }}>
                                        <View>
                                            <Text style={{ fontWeight: 'bold', fontSize: 13, color: '#cf2a3a' }}>{`${trip.departure}`}</Text>
                                            <Text style={{ fontWeight: 'bold', fontSize: 12 }}>{`${trip.mobile_code} [ ${trip.code} ]`}</Text>
                                        </View>
                                    </TouchableOpacity>
                                ))}
                            </View>
                            <View style={{ marginTop: 20 }}>
                                <Text style={{ fontWeight: 'bold', fontSize: 16, color: '#cf2a3a' }}> {trips?.[0].trip_id} </Text>
                            </View>
                        </>
                    )}
                </View>
