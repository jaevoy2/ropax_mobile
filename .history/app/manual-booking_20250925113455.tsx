...
export default function ManualBooking() {
    ...
    const handleSaveTrip = (selectedTrip: string, trip_id: number, origin: string, destination: string, code: string, web_code: string, departureTime: string) => {
        setLoading(true);            
        setTimeout(() => {
    ...
    return (
        <View style={{ backgroundColor: '#f1f1f1' }}>
            ...
            <ScrollView refreshControl={ <RefreshControl refreshing={refresh} onRefresh={handleRefresh} style={{ borderColor: '#CF2A3A' }} /> } >
                ...
                <View style={{ padding: 20 }}>
                    ...
                    <View>
                        {contentLoading == true ? (
                            ...
                            <>
                            {trips?.map((trip) => (
                                <TouchableOpacity onPress={() => handleSaveTrip(trip.vessel, trip.trip_id, trip.route_origin, trip.route_destination, trip.code, trip.web_code, trip.departure_time)} key={trip.trip_id} style={{ paddingHorizontal: 15, paddingVertical: 25, backgroundColor: '#fff', borderRadius: 10, marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
