...
export default function Forms({ errorForm }: FormProps) {
    const { vessel_id, destination, origin, totalFare, setTotalFare } = useTrip();
    ...
    const getPassengerFare = async (passType_id: number, accomm_id: number, seatNum: string | number) => {
        console.log(passType_id, accomm_id, vessel_id);
        try{
    ...
    const hasInfantChecker = (seat: number | string, passIndex: number, hasInfantValue: boolean, type_id: number) => {
        const hasPasses = passengers.some((p) => p.accommodation == 'Passes');
    ...
    const onFareInput = (passIndex: number, seatNum: string | number | undefined, fare: string) => {
        const hasPasses = passengers.some((p) => p.accommodation == 'Passes');
    ...
    const onFieldInput = (passIndex: number, seatNum: string | number | undefined, inputKey: string, value: string | number) => {
        const hasPasses = passengers.some((p) => p.accommodation == 'Passes');
            ...
            <View style={{ height: '82%', marginTop: 10 }}>
                <ScrollView>
                    {passengers.map((p, index) => (
                        <View key={`${p.accommodationID}-${index}`} style={{ position: 'relative', borderColor: errorForm.includes(p.seatNumber ?? '') ? '#cf2a3a' : '#B3B3B3', borderWidth: 1, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 10, marginBottom: 20, backgroundColor: '#fff' }}>
                            {passengers.some((p) => p.accommodation == 'Passes' && index != 0) && (
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                {!passengers.some((p) => p.accommodation == 'Passes') && (
                                    <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                                ...
                                <View style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                                    ...
                                    <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                        <Text style={{ fontSize: 16 }}>₱</Text>
                                        <TextInput onChangeText={(text) => onFareInput(index, p.seatNumber, text)} value={String(p.fare ?? '')} keyboardType={'numeric'} placeholder='00.00' style={{ fontWeight: 'bold' }} />
                                    </View>
                                </View>
                            </View>
                            {!passengers.some((p) => p.accommodation == 'Passes') && (
                                        ...
                                        {typeLoading == true ? (
                                            ...
                                            <>
                                                {passengerType?.filter((t) => t.name != 'Infant' && t.name != 'Passes')
                                                    .map((type) => (
                                                    <TouchableOpacity onPress={() => {updatePassenger(p.seatNumber!, 'passType', type.name), updatePassenger(p.seatNumber!, 'passType_id', type.id), getPassengerFare(type.id, p.accommodationID!, p.seatNumber!), updatePassenger(p.seatNumber!, 'passTypeCode', type.code)}} key={type.id} style={{ backgroundColor: p.passType == type.name ? '#cf2a3a' : 'transparent', borderColor: '#cf2a3a', borderWidth: 1, paddingVertical: 4, paddingHorizontal: 18, borderRadius: 5  }}>
                            ...
                            <View style={{ marginTop: 10 }}>
                                ...
                                <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                    <TextInput onChangeText={(text) => onFieldInput(index, p.seatNumber, 'name', text)} placeholder='Last Name, First Name' style={{ fontSize: 13 }} />
                                </View>
                            </View>
                            <View style={{ marginTop: 5, flexDirection: 'row', gap: 8, alignItems: 'flex-end' }}>
                                <View style={{ width: '40%' }}>
                                    ...
                                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                        <TextInput onChangeText={(text) => onFieldInput(index, p.seatNumber, 'age', Number(text))} keyboardType='numeric' placeholder='Age' style={{ fontSize: 13 }} />
                                    </View>
                                </View>
                                <View style={{ width: '56%', }}>
                            ...
                            <View style={{ marginTop: 5, flexDirection: 'row', gap: 8 }}>
                                <View style={{ width: '40%' }}>
                                    ...
                                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                        <TextInput onChangeText={(text) => onFieldInput(index, p.seatNumber, 'nationality', text)} defaultValue='Filipino' style={{ fontSize: 13 }} />
                                    </View>
                                </View>
                                <View style={{ width: '57.5%' }}>
                                    <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#545454' }}>Address:</Text>
                                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                        <TextInput onChangeText={(text) => onFieldInput(index, p.seatNumber, 'address', text)} placeholder='Address' style={{ fontSize: 13 }} />
                                    </View>
                                </View>
                            </View>
