...
export default function Forms({ errorForm }: FormProps) {
    ...
    const hasCargoChecker = (seat: number | string, passIndex: number, hasCargo: boolean) => {
        const hasPasses = passengers.some((p) => p.passType == 'Passes');
        const currentValue = !hasCargo;

        if(!hasPasses) {
            updatePassenger(seat, 'hasCargo', currentValue);
    
            if(!currentValue) {
                updatePassenger(seat, 'cargo', []);
            }else {
                addPaxCargo(seat, { cargoFare: '' });
            }
        }else {
            updatePassenger(passIndex, 'hasCargo', currentValue);
    
            if(!currentValue) {
                updatePassenger(passIndex, 'cargo', []);
            }else {
                addPaxCargo(passIndex, { cargoFare: '' });
            }
        }
    }
                ...
                {passengers.map((p, index) => (
                    <View key={index + 1} style={{ position: 'relative', borderColor: errorForm.includes(p.seatNumber ?? '') ? '#cf2a3a' : '#B3B3B3', borderWidth: 1, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 10, marginBottom: 20, backgroundColor: '#fff' }}>
                        ...
                        <View style={{ marginTop: 5, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                            ...
                            <TouchableOpacity onPress={() => hasCargoChecker(p.seatNumber!, index, p.hasCargo! )}
                                style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15 }}>
