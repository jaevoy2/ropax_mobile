...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, letter, skip = 0, count = 0, onSeatSelect, type, accomm_id, bookedSeats }) => {
    ...
    useEffect(() => {
        channel.on('broadcast', { event: 'seat_select' }, payload => {
            setSeatSelectionChannel((prev) => [...prev, payload])
        })
    }, []);
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => {
                ...
                return(
                    <TouchableOpacity disabled={!!booked || isPassenger || seatSelectionChannel.includes(seat)} key={seat} onPress={() => onSeatSelect?.(`${letter}${seat}`, type, accomm_id)} 
                    style={{ width: 33, height: 33, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, 
                    backgroundColor: booked ? booked.station.color : isPassenger ? '#BA68C8' : touristSeat.includes(`${letter}${seat}`) ? '#E6E2C6' : seatSelectionChannel ? '#F9F9F9' : 'transparent',
                    alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                        <Text style={{ fontSize: 10.5, fontWeight: 'bold', 
                            color: booked || isPassenger ? '#fff' : seatSelectionChannel ? '#d3d3d3ff' : '#000' }}>
