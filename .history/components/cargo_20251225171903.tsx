...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    const { cargo_types, brands, specifications, parcel_categories, cargos } = cargoProperties.data;
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <View style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                                    ...
                                    <View style={{ borderColor: '#FFC107', backgroundColor: '#ffc10727', borderWidth: 2, borderRadius: 5, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                        ...
                                        <TextInput onChangeText={(text) => updateCargo(0, 'cargoFare', Number(text))} keyboardType={'numeric'} placeholder='00.00' style={{ fontWeight: 'bold', textAlign: 'right' }} />
                                    </View>
                                </View>
                            </View>
                                    ...
                                    {passengers[0].cargo.cargoType == 'Rolling Cargo' ? (
                                        <View style={{ marginTop: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                            <View style={{ width: '53%' }}>
                                                ...
                                                <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                    <Dropdown onChange={(item) => {updateCargo(0, 'brand', item.label), updateCargo(0, 'brandID', item.value), setSelectdBrand(item.value)}} value={selectedBrand} data={brands.map((b: any) => ({ label: b.name, value: b.id })) } labelField="label" valueField="value" placeholder="Select Brand" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                        containerStyle={{
                                                            alignSelf: 'flex-start',
                                                            width: '45%',
                                                        }}
                                            ...
                                            <View style={{ width: '45%' }}>
                                                ...
                                                <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                    <Dropdown onChange={(item) => {updateCargo(0, 'specification', item.label), updateCargo(0, 'specificationID', item.value), setSelectedCC(item.value)}} value={selectedCC} data={specifications.map((specs: any) => ({ label: specs.cc, value: specs.id }))} labelField="label" valueField="value" placeholder="Select CC" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                        containerStyle={{
                                                            alignSelf: 'flex-start',
                                                            width: '38%',
                                                        }}
                                        ...
                                        <View style={{ marginTop: 5 }}>
                                            ...
                                            <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                <Dropdown onChange={(item) => {updateCargo(0, 'parcelCategory', item.label), updateCargo(0, 'parcelCategoryID', item.value), setSelectedParcelCategory(item.value)}} value={selectedParcelCategory} data={parcel_categories.map((category: any) => ({ label: category.name, value: category.id })) } labelField="label" valueField="value" placeholder="Select Category" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                    containerStyle={{
                                                        alignSelf: 'flex-start',
                                                        width: '85%',
                                                    }}
                                        ...
                                        <View style={{ marginTop: 5 }}>
                                            <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#545454' }}>Animal Type:</Text>
                                            <View style={{ borderColor: '#B3B3B3', borderWidth: 1, borderRadius: 5 }}>
                                                <Dropdown onChange={(item) => {updateCargo(0, 'parcelCategory', item.label), updateCargo(0, 'parcelCategoryID', item.value), setSelectedParcelCategory(item.value)}} value={selectedParcelCategory} data={parcel_categories.map((category: any) => ({ label: category.name, value: category.id })) } labelField="label" valueField="value" placeholder="Select Animal Type" style={{ height: 40, width: '100%', paddingHorizontal: 10 }}
                                                    containerStyle={{
                                                        alignSelf: 'flex-start',
                                                        width: '85%',
                                                    }}
