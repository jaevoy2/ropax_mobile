...
export default function Forms({ errorForm }: FormProps) {
    ...
    //         const passenger = passengers.filter(p => p.hasCargo == true);

    //         const prevCargoProp = passenger.filter(p => p.hasCargo)
    //             .flatMap(p => p.cargo)
    //             .find(cargo => cargo.id == passenger.filter(p => p.cargo).find((c, index) => index));
