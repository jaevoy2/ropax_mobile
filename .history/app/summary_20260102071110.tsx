            ...
            <View style={{ paddingHorizontal: 10, paddingTop: 20 }}>
                    ...
                    <View style={{ borderBottomColor: '#c9c9c9ff', borderBottomWidth: 1, paddingVertical: 10 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                {paxCargoProperty.map((cargo: any) => (
                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <View style={{ marginLeft: 15, flexDirection: 'row', gap: 3 }}>
                                            <Text style={{ fontSize: 12, color: '#858585ff' }}>
                                                { cargo.cargoType == 'Rolling Cargo' ? `${cargo.cargoBrand} ${cargo.cargoSpecification}` : cargo.parcelCategory}
                                            </Text>
                                            <Text style={{ fontSize: 12, color: '#858585ff' }}>{`(${cargo.cargoType})`}</Text>
                                            <Text style={{ fontSize: 12, color: '#858585ff' }}>{cargo.quantity >  1 ? `(x${cargo.quantity})` : ''}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                            <Text style={{ fontSize: 12, color: '#858585ff' }}>₱ </Text>
                                            <Text style={{ fontSize: 12, color: '#858585ff' }}>{cargo.cargoAmount.toLocaleString('en-PH', { minimumFractionDigits: 2,  maximumFractionDigits: 2})}</Text>
                                        </View>
                                    </View>
                                ))}
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                                <Text style={{ fontSize: 12, color: '#858585ff' }}>₱ </Text>
                                <Text style={{ fontSize: 12, color: '#858585ff' }}>{totalAndNote.totalAmount.toLocaleString('en-PH', { minimumFractionDigits: 2,  maximumFractionDigits: 2})}</Text>
                            </View>
                        </View>
                    </View>
                )}
                <View style={{ borderBottomColor: '#c9c9c9ff', borderBottomWidth: 1, paddingVertical: 10 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {/* <Text style={{ fontSize: 16, fontWeight: '900', color: '#cf2a3a' }}>₱ {!cargoOnlyProperty.cargoAmount ? `${totalFare}.00` : cargoOnlyProperty.cargoAmount}</Text> */}
                    </View>
                </View>
