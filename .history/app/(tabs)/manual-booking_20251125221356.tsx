    ...
    const handleFetchTotalBookings = async (trip_id: number | null) => {
        try {
            ...
            if(!totalBookingFetch.error) {
                ...
                const totalBookingAccomTypes: AccommodationProps[] = totalBookingFetch.accomTypes.map((a: any) => ({
                    accomtTypeID: a.id,
                                            ...
                                            {totalBookings?.map((tb, index) => (
                                                <View key={index} style={{ paddingBottom: 10, borderBottomColor: '#b4b4b4ff', borderBottomWidth: 1 }}>
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                                        {accomTypes?.map((accom) => (
                                                            <>
                                                                <Text style={{ fontWeight: 'bold', width: '50%' }}>{accom.accomType}</Text>
                                                
                                                                {tb.accommodationGroup
                                                                .filter(a => a?.accommodation == accom.accomType)
