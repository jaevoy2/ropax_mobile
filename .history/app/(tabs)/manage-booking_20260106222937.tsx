        ...
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <View style={{ justifyContent: 'center', paddingTop: 20, paddingHorizontal: 20 }}>
                ...
                <FlatList data={bookings} keyExtractor={(booking) => booking.referenceNumber} renderItem={({ item }) => (
                    <View style={{ borderColor: '#B3B3B3', borderWidth: 1, backgroundColor: '#fff', borderRadius: 8, padding: 8, marginBottom: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View>
                            <Text style={{ fontWeight: 'bold', fontSize: 16 }}>{item.name}</Text>
                            <Text style={{ fontSize: 10, color: '#cf2a3a', fontWeight: 'bold' }}>{`${item.referenceNumber} | ${item.route} | ${item.departureTime}`}</Text>
                        </View>
