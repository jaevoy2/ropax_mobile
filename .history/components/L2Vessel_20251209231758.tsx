...
import { broadcastSeatSelection, channel } from '@/utils/channel';
...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, letter, skip = 0, count = 0, onSeatSelect, type, accomm_id, bookedSeats }) => {
    ...
    useEffect(() => {
        const listen = channel.on('broadcast', { event: 'seat_select' }, payload => {
            setSeatSelectionChannel((prev) => [...prev, payload.payload.seat])
        });

        const seatListenRemoval = channel.on('broadcast', { event: 'seat_removal' }, payload => {
            setSeatSelectionChannel(prev => prev.filter(s => s !== payload.payload.seat))
        })

        return () => {
            listen.unsubscribe();
            seatListenRemoval.unsubscribe();
        }
    }, []);
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => {
                ...
                return(
                    ...
                    backgroundColor: booked ? booked.station.color : isPassenger ? '#BA68C8' : touristSeat.includes(`${letter}${seat}`) && !seatSelectionChannel.includes(`${letter}${seat}`) ? '#E6E2C6' : seatSelectionChannel.includes(`${letter}${seat}`) ? '#e1bbe7ff' : 'transparent',
                    alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                        <Text style={{ fontSize: 10.5, fontWeight: 'bold', 
                            color: booked || isPassenger ? '#fff' : seatSelectionChannel.includes(`${letter}${seat}`) ? '#d3d3d3ff' : '#000' }}>
