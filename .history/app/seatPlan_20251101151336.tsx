...
export default function SeatPlan() {
    ...
    const seatSnapPoints = useMemo(() => {
        if(passengers.length == 0) return seatSheetRef.current?.close();
        if(passengers.length > 0) return ["26%"];
        
        const containerPercent = (containerHeight / height) * 100;
        if(passengers.length > 6) {
            return[
                Math.min(containerPercent, 100) + "%",
                Math.min(containerPercent + 10, 100) + "%",
                "90%"
            ]
        }
    }, [containerHeight, passengers.length]);
    ...
    return (
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={{ height: '100%', overflow: 'hidden' }}>
                ...
                <BottomSheet ref={seatSheetRef} snapPoints={seatSnapPoints} index={passengers.length > 0 ? 0 : -1} bottomInset={1} enableContentPanningGesture={false} handleComponent={null}>
