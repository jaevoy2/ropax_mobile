...
export default function ManualBooking() {
  ...
  .map((accom, accomIndex) => (
    <View key={accomIndex}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
        {paxTypes?.map((type) => {
          const matches = accom.passenger.filter(
            (pax) => String(pax.type).trim().toLowerCase() === String(type.paxType).trim().toLowerCase()
          );

          // either show matches or a placeholder "0"
          return (
            <View key={type.paxTypeID ?? type.paxType} style={{ marginRight: 8 }}>
              {matches.length > 0 ? (
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <Text style={{ color: '#5c5c5cff', fontSize: 12 }}>{type.paxType}: </Text>
                  <Text style={{ color: '#5c5c5cff', fontSize: 12 }}>0</Text>
                </View>
              )}
            </View>
          );
