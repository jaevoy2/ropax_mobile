    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => (
                ...
                backgroundColor: passengers.some((p) => bookedSeats?.find((s) => s.seat_no == String(`${letter}${seat}`))?.station.name === 'Hilongos Port'
                    ? '#01760B'
                    : bookedSeats?.find((s) => s.seat_no == `${letter}${seat}`)?.station.name === 'Hilongos Shop'
                    ? '#64B5F6' :
                    p.seatNumber == String(`${letter}${seat}`)) ? '#BA68C8' : touristSeat.includes(`${letter}${seat}`) ? '#E6E2C6' : 'transparent',
