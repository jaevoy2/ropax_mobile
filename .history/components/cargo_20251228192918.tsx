...
export default function CargoComponent({ dateChange }: {dateChange: string} ) {
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <Animated.View style={{ paddingHorizontal: 15, paddingTop: 10, transform: [{ translateX: tripsAnimation }], position: 'absolute', width }}>
                {formLoading == true ? (
                    <View style={{ height: height, justifyContent: 'center', alignSelf: 'center' }}>
                        <ActivityIndicator size={'large'} color={'#cf2a3a'} />
                    </View>
                ) : (
