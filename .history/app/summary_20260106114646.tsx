...
export default function PaymentSummary() {
    ...
    const handleConfirmation = async () => {
        ...
        try {
            console.log(passengers.filter(p => p.cargo.map(c => c.cargoOptionID)))
                                ...
                                p.hasInfant && p.infant?.map((i, index) => (
                                    <View key={`${p.accommodationID}-${index}`} style={{ marginBottom: 3 }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <Text style={{ fontSize: 12, width: '40%', fontWeight: '400' }}>{`${i.name?.split(',')[1]?.trim().charAt(0)}. ${i.name?.split(',')[0]} (Infant)`}</Text>
