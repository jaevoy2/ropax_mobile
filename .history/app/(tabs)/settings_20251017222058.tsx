...
export default function GenSettings() {
    ...
    const handleSaveStation = async () => {
        ...
        try {
            ...
            if(saveStation) {
                AsyncStorage.setItem('stationID', saveStation.station.id);
                            ...
                            <>
                                ...
                                <View style={{ flexDirection: 'column', gap: 8, marginTop: 10 }}>
                                    {stationOptions?.map((option) => (
                                        <TouchableOpacity onPress={() => setSelectedStation(option.id)} key={option.id} style={{ paddingHorizontal: 8, paddingVertical: 10, borderWidth: 1, borderColor: '#cf2a3a', borderRadius: 5, backgroundColor: selectedStation == option.id ? '#cf2a3a' : 'transparent' }}>
            ...
            <View style={{ padding: 10 }}>
                <View style={{ paddingHorizontal: 15, paddingVertical: 25, backgroundColor: '#fff', borderRadius: 10, marginTop: 5, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <View style={{ flexDirection: 'column', alignItems: 'flex-end', marginTop: 10 }}>
                        ...
                        {user?.station ? (
                            <Text style={{ fontSize: 12, color: '#cf2a3a' }}>{user?.id}</Text>
                        ) : (
