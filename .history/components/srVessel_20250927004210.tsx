...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false, onSeatSelect, type, accomm_id, bookedSeats }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' }}>
            {items.map((seat) => (
                <TouchableOpacity disabled={passengers.some((p) => p.seatNumber == Number(seat)) || bookedSeats?.some((s) => s.seat_no == seat)} onPress={() => onSeatSelect?.(seat, type, accomm_id)} key={seat}
                ...
                alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                    <Text style={{ fontSize: 14, fontWeight: 'bold', color: bookedSeats?.find((s) => s.seat_no == seat) ? '#fff' : passengers.some((p) => p.seatNumber == Number(seat)) ? '#fff' : '#000' }}>{seat}</Text>
                </TouchableOpacity>
            ))} 
        </View>
    )
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 10, width: '80%', alignSelf: 'center', }}>
                    ...
                    {seatColumn1.map((seat) => ( 
                        ...
                        : 'transparent' }}> 
                            <Text style={{ textAlign: 'center', fontWeight: 'bold', 
                                color: bookedSeats.find((s) => s.seat_no == seat) ? '#fff' : passengers.some((p) => p.seatNumber == String(seat)) ? '#fff' : '#000' }}>
                                {bookedSeats?.find((s) => s.seat_no == seat)?.seat_no ?? seat}
                            </Text> 
                        </TouchableOpacity>
                    ))}
                </View>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 90, gap: 4, justifyContent: 'center' }}>
                    ...
                    {seatColumn2.map((seat) => ( 
                        ...
                        : passengers.some((p) => p.seatNumber == String(seat))
                        ? '#BA68C8' : '#E6E2C6' }}> 
                            <Text style={{ textAlign: 'center', fontWeight: 'bold', color: bookedSeats.find((s) => s.seat_no == seat) ? '#fff' : passengers.some((p) => p.seatNumber == String(seat)) ? '#fff' : '#000' }}>{seat}</Text> 
                        </TouchableOpacity>
                    ))}
                </View>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 90, gap: 4, justifyContent: 'center' }}>
