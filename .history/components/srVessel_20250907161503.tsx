...
const SeatPlan: React.FC<SeatProps> = ({ start, limit, skipPattern = false }) => {
    ...
    return (
        <View>
            {items.map((seat) => (
                <TouchableOpacity key={seat} style={{ width: 40, height: 40, borderColor: '#A9A9B2', borderWidth: 1, borderRadius: 3, backgroundColor: touristSeat.includes(seat) ? '#E6E2C6' : 'transparent', alignItems: 'center', justifyContent: 'center', margin: 2 }}>
                    <Text>{seat}</Text>
                </TouchableOpacity>
            ))} 
        </View>
