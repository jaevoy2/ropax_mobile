...
export default function CargoComponent() {
    ...
    const animateTop = formSheetAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [0, -215]
    });
    ...
    return (
        <View style={{ height: height }}>
            ...
            <Animated.View style={{ opacity: formSheetAnim, paddingHorizontal: 15, backgroundColor: '#f1f1f1', height: '100%', transform: [{ translateY: animateTop }] }}>
                {formLoading == true ? (
                    ...
                    <View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <View>
                                <Text style={{ color: '#cf2a3a', fontSize: 24, fontWeight: '900' }}>{selectedVessel}</Text>
                                <Text style={{ color: '#747373ff', fontSize: 12, marginTop: -5 }}>{timeWithRoute}</Text>
                            </View>
                        </View>
                    </View>
                )}
